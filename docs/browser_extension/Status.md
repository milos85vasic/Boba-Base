...
| 5 | Tab-group batch (`chrome.tabGroups` → per-tab scan → batch send) | IN-PROGRESS | **Standalone module landed** (Session 11): `src/tabgroups/index.ts` (dedupe across a tab group + dispatch batched send), `tabgroups.test.ts` 13 green. **PENDING:** integration into `background/index.ts` (the `MENU_SEND_GROUP` handler) + the `tabGroups`/`tabs` manifest permissions (least-privilege review required before adding) |
