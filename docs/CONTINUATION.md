...
**Revision:** 27
**Last modified:** 2026-08-20T12:17:48Z

## TERMINAL STATE (this write) — 45 commits landed since Rev 26 (df7bc41 → e0d60ab): constitution v1.3.0 drift-review, BOB-126 fallout closed, quality-tooling 3-agent fan-out

- **Staleness measured, not assumed (§11.4.6)**: Rev 26 was written at `df7bc41` (2026-08-19T17:25:34+02:00). HEAD is now `e0d60ab` (2026-08-20T12:01:57+02:00) — **45 commits, ~19h elapsed**, not the "53 days / 24+ commits" figure this session's own workable item (BOB-081) was originally filed under; that figure described an earlier, since-superseded staleness state (Rev 25 or before). `git status --porcelain` on `main` at write time shows the tree otherwise clean except an in-flight parallel edit (see below).
- **3 remotes IN SYNC at HEAD `e0d60ab`** (github/origin/upstream — 0 ahead / 0 behind, verified via `git fetch --all` + per-remote `git log --oneline <remote>/main..HEAD` and the reverse).
- **In-flight, NOT part of this write**: `scripts/pre_build_verification.sh` (modified, uncommitted) + two new untracked files `scripts/lib/export_staleness.sh` and `tests/unit/test_export_staleness_oracle.sh`. These are consistent with the `TODO(EXPORT-SYNC-CONTENT-HASH)` migration plan e0d60ab left explicitly open (mtime-based export-staleness check is not reproducible across fresh clones — nanosecond mtime deltas + `.html` sorting before `.md` on checkout produced false-STALE on two fresh-clone runs). Owned by a concurrent session/agent per this task's file-scope restriction; not committed, not evaluated here.
- **Constitution submodule bumped to v1.3.0** (`78f18e8`, pointer now `502586c`) — full drift-review closing ~4 months of doc/code drift (container roster 2→5 services, plugin roster 12→43, cookies-file autoload, Hard Stop #3 orchestration ownership, Go webui-bridge profile, new Principles XII Anti-Bluff Captured Evidence + XIII Host-Session Safety) plus new universal anchor **§11.4.263 process-group signal-safety mandate** (the BOB-126 `killpg(pgid≤1)` forensic anchor, generalised across Python/Go/Rust/Bash/C).
- **BOB-126 fallout fully closed**: BOB-127 (evidence_path fix), BOB-128 (killpg-carrier guard collision with the new §11.4.263 gate — fixed `be36f32`; `CM-TEST-MOCK-PID-PATCHED-WHEN-REAL-PID` landed as pre_build invariant 29), BOB-129 (**real production slowapi/starlette rate-limiting defect** found by a Task-105 subagent and fixed on production endpoints, before/after evidence captured — `44f3bbe`/`e141d5b`), BOB-130 (pre_build timeout raised for a genuinely CPU-bound 93s fixture), BOB-131/132/133/135 registered. All eight registered/closed in the DB at `d9955d6`, exports regenerated.
- **`5223f48` — 5 caveats closed with RED→GREEN evidence, not assertion**: (1) `compute-badges.sh` — a bare `awk` filter had been corrupting README by overwriting its own documenting prose line, plus a non-idempotent `docs/TESTING.md` regenerator that had accumulated to 7 stray blank lines; both fixed + guarded (`test_compute_badges_carrier_match.sh`). (2) A `.gitignore` rule was shadowing 58 TRACKED files (including the §11.4.109 hook config `.claude/settings.json` and production `config/merge-service/*`), which made `git add` exit non-zero under `set -euo pipefail` — i.e. it could silently break the mandated commit entrypoint; unshadowed + guarded (`test_tracked_files_are_addable.sh`). (3) 2 dead constitution-mirror push URLs removed (repos verified non-existent via `gh`) — multi-URL fan-out was aborting on first failure and starving URLs 5–10; fan-out now completes 8/8. (4) QA discovery ledger: 2 new out-of-band entries with escape-audit + new-check, 18→20. (5) `speckit.git.initialize` root-cause fixed (an `.specify/extensions.yml` config omission). **Bonus finding**: `tests/unit/*.sh` were being executed by nothing (`ci.sh` only collects `test_*.py`) — new pre_build invariant 30 `CM-BASH-UNIT-TESTS-EXECUTED` wires them in; first run immediately surfaced 3 latent FAILs, quarantined by name as tracked debt per §11.4.226 (predicted exactly this).
- **`e0d60ab` (HEAD) — 3-agent fan-out, each verified by the controller, none accepted on claim (§11.4.262)**: Agent 1 fixed a ~50%-flaky DOCX-export test (GNU vs BSD `stat -f` syntax divergence — portable `portable_mtime()` helper landed, validated against real GNU/BSD polarity). Agent 2 stopped two challenge scripts from bumping mtimes of real tracked files on backup/restore (`cp`→`cp -p`), verified byte-identical + unchanged mtime/sha256 post-run. Agent 3 widened the mutation-residue "no-trace" assertion from 3 tracked paths to **3958** (main repo + constitution submodule — `git ls-files` does not descend into submodules), replacing a whole-second `stat -c%Y` compare (proven blind to same-second mutate+restore) with a `bash -nt`-vs-marker-file compare, plus three new §11.4.201 refusal branches (empty corpus / unenumerable legacy path / unproven control needle) and a 6-quadrant polarity proof against the shipped bytes. Controller work: 8 stale `.playwright-mcp` tracked snapshots untracked (kept on disk, zero consumers verified, operator-approved per §11.4.122); `docs/BOBA_DATABASE.md`'s dangling `§ Key Rotation` anchor fixed + the rotation procedure honestly marked NOT executable (no `rotate-key`/`envfile-replace` in source — corrects an earlier overclaim). QA ledger rev 15, entries 20→22, gate 32/32 passed. **One item deliberately left OPEN, not closed**, per §11.4.226 evidence-class discipline: `EXPORT-SYNC-MTIME-NOT-REPRODUCIBLE-ON-FRESH-CLONE` — see the in-flight files noted above.
- **Standing follow-ups from Rev 26 — status re-checked this write**: "helixqa nested dirty state" — **now CLEAN** (`git status --porcelain` inside `submodules/helixqa` returns empty at write time; superseded, no longer open). "Retrospective correction of BOB-116/120/123 incident docs" — **DONE** (`b696873`, landed same day as Rev 26, ~2 min after). "Constitution/gitverse push rejected by org policy" — **still open**, no new evidence found this write; remains a tracked §11.4.197 followup. "§11.4.185 manual QA before release tag" — **UNKNOWN**: no commit in the `df7bc41..HEAD` range references manual QA or §11.4.185; not confirmed done, not confirmed still pending — treat as still owed until evidence surfaces.
- **Open work-item tracker snapshot** (`docs/workable_items.db`, queried live at write time): **50 non-terminal items** — 43 Queued, 3 In progress (BOB-066 Lava P3 upstream-proxy wiring, BOB-104 CodeGraph nested-.gitignore regression challenge, BOB-111 real rate limiting for boba's 3 public HTTP endpoints), 4 Operator-blocked (BOB-008 RuTracker CAPTCHA, BOB-077 second Auto-commit host identification, BOB-101 `--profile go` parity goal, BOB-102 LAN-exposure threat model — all await an explicit operator decision). The RD2-series backlog (BOB-065/068/069/070/074/076/078–100) covering egress diagnosis, the unattributed Auto-commit mechanism, DDoS/scaling/UX test-type coverage, and README doc-link completeness is still fully queued. Full enumerated list in `docs/Issues.md` / `docs/Issues_Summary.md` (§11.4.57/§11.4.212).
- **This entry closes BOB-081** (RD2-14 — author this Session-N CONTINUATION.md entry). Test-count badges currently read 5358 Python / 371 frontend tests collected (`README.md`, refreshed at `c37727b`).

## TERMINAL STATE (previous write) — 7-forced-logout chain CLOSED + universal §11.4.263 anchor landed
