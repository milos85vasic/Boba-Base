...
Contributing-section bullets. Last regenerated: 2026-08-19T11:45:29Z.

| Suite | Directory | Real count | Method |
|---|---|---|---|
| Python (whole `tests/` tree) | `tests/` | **5262** | `pytest --collect-only -q` |
| Python unit | `tests/unit/` | **4400** | `pytest --collect-only -q` |
| Python integration | `tests/integration/` | **285** | `pytest --collect-only -q` |
...
| HelixQA Challenges | `challenges/scripts/*.sh` | **34** | `ls challenges/scripts/*.sh \| wc -l` |
| Pre-build invariants | `scripts/pre_build_verification.sh` | **26** | max total of every `[N/N]` progress label |
