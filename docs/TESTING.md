...
Contributing-section bullets. Last regenerated: 2026-09-02T10:13:45Z.

| Suite | Directory | Real count | Method |
|---|---|---|---|
| Python (whole `tests/` tree) | `tests/` | **5857** | `pytest --collect-only -q` |
| Python unit | `tests/unit/` | **4724** | `pytest --collect-only -q` |
...
| Frontend (Vitest) | `frontend/src/**/*.spec.ts` | **2742** | collected (vitest list --run) |
| HelixQA Challenges | `challenges/scripts/*.sh` | **38** | `ls challenges/scripts/*.sh \| wc -l` |
| Pre-build invariants | `scripts/pre_build_verification.sh` | **55** | max total of every `[N/N]` progress label |
