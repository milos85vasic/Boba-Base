...
Contributing-section bullets. Last regenerated: 2026-09-23T16:01:14Z.

| Suite | Directory | Real count | Method |
|---|---|---|---|
| Python (whole `tests/` tree) | `tests/` | **6195** | `pytest --collect-only -q` |
| Python unit | `tests/unit/` | **4974** | `pytest --collect-only -q` |
| Python integration | `tests/integration/` | **308** | `pytest --collect-only -q` |
| Python e2e | `tests/e2e/` | **35** | `pytest --collect-only -q` |
| Python contract | `tests/contract/` | **9** | `pytest --collect-only -q` |
| Frontend (Vitest) | `frontend/src/**/*.spec.ts` | N/A | vitest list --run produced no parseable test lines |
| HelixQA Challenges | `challenges/scripts/*.sh` | **38** | `ls challenges/scripts/*.sh \| wc -l` |
| Pre-build invariants | `scripts/pre_build_verification.sh` | **57** | max total of every `[N/N]` progress label |
