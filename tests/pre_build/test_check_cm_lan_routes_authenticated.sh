...
trap cleanup EXIT
# ROUND 13 (MINOR-4, §11.4.14) — `trap ... EXIT` CANNOT RUN UNDER SIGKILL.
# A run killed by the tool-call ceiling (or by an OOM reap) skips cleanup
# entirely, and the leaked trees are not small: one such kill left 32 GB of
# `lanauth-meta-*` behind. A trap alone is therefore a cleanup that works only
# on the paths that did not need it.
# The sweep is AGE-GATED so it can never reap a CONCURRENTLY RUNNING harness's
# live directory (§11.4.84 quiescence): only trees whose mtime is older than
# the gate — far longer than any run of this harness — are removed, and the
# CURRENT $TMPROOT is excluded by name as a belt for those braces. Failures are
# swallowed on purpose: a stale-tree sweep must never be able to fail the gate
# it is housekeeping for.
_r13_sweep_stale_tmproots() {
  local base; base="$(dirname "$TMPROOT")"
  [[ -d "$base" ]] || return 0
  nice -n 19 find "$base" -maxdepth 1 -type d -name 'lanauth-meta-*'        ! -name "$(basename "$TMPROOT")" -mmin +120        -exec rm -rf {} + 2>/dev/null || true
}
_r13_sweep_stale_tmproots
...
echo
echo "=== round 13 (BLOCKING): the INVERTED primitive — absence DISARMS a refusal ==="
# Round 11 closed "a selector can split at a dot" for every construct that
# GRANTS CREDIT. Round 12 found the audit sentence that followed it was FALSE:
# `gin.New()/gin.Default()` carries NO CREDIT TO DROP, so making it invisible
# does not degrade loudly — it DISARMS the `len(engines) > 1` refusal that is
# the precondition for file-scoped `.Use` crediting, and the gate exits 0 over
# an unauthenticated mutating LAN route.
#
# THE CLASS: silence is safe for a credit-carrier and unsafe for a
# refusal-armer. Every fixture below is a spelling `gofmt -e` accepts; the two
# marked STABLE survive gofmt byte-identical.
r13_gin() {  # r13_gin <name> — WHOLE file body arrives on STDIN
  local d="$TMPROOT/$1"; mkdir -p "$d/cmd"
  { printf 'package main\n\n'; cat; } > "$d/cmd/main.go"
  mk_policy_go "$d" gin cmd '"AuthMW"' '  - service: svc
    method: GET
...
r13_gomux() {  # r13_gomux <name> — WHOLE file body arrives on STDIN
  local d="$TMPROOT/$1"; mkdir -p "$d/jackettapi"
  { printf 'package jackettapi\n\nimport "net/http"\n\n'; cat; } > "$d/jackettapi/router.go"
  mk_policy_go "$d" gomux jackettapi '"WithAuth"' '  - service: svc
    method: ANY
    path: /ok
    class: public-by-design
    justification: "Probe."'
...
# --- the BLOCKING itself: gofmt-STABLE split engine declaration -------------
r13_gin r13-eng-split-newline <<'R13EOF'
func buildOps() {
	r := gin.New()
	r.Use(AuthMW())
	r.GET("/ok", api.OK)
...
# --- every same-line whitespace position the round-6 spelling was rigid at ---
r13_gin r13-eng-ws-around <<'R13EOF'
func buildOps() {
	r := gin.New()
	r.Use(AuthMW())
	r.GET("/ok", api.OK)
...
r13_gin r13-eng-ws-paren <<'R13EOF'
func buildOps() {
	r := gin.New()
	r.Use(AuthMW())
	r.GET("/ok", api.OK)
...
# --- forward-walk over blanks AND comments, the round-11 property, re-pinned
#     one construct over: an immediate-next-line check would drop this.
r13_gin r13-eng-split-comment <<'R13EOF'
func buildOps() {
	r := gin.New()
	r.Use(AuthMW())
	r.GET("/ok", api.OK)
...
# --- BOTH engines split: zero counted, so the refusal was doubly disarmed ----
r13_gin r13-eng-both-split <<'R13EOF'
func buildOps() {
	r := gin.
		New()
	r.Use(AuthMW())
...
# --- the SAME class on the NAMING axis: an aliased import ------------------
r13_gin r13-eng-alias-two <<'R13EOF'
...
r13_gin r13-eng-alias-split <<'R13EOF'
import (
	"net/http"
...
# --- a DOT import leaves no selector at all: refuse, never credit blind -----
r13_gin r13-eng-dot-import <<'R13EOF'
...
run_case r13-eng-split-newline  1 "$TMPROOT/r13-eng-split-newline"
run_case r13-eng-ws-around      1 "$TMPROOT/r13-eng-ws-around"
run_case r13-eng-ws-paren       1 "$TMPROOT/r13-eng-ws-paren"
run_case r13-eng-split-comment  1 "$TMPROOT/r13-eng-split-comment"
run_case r13-eng-both-split     1 "$TMPROOT/r13-eng-both-split"
run_case r13-eng-alias-two      1 "$TMPROOT/r13-eng-alias-two"
run_case r13-eng-alias-split    1 "$TMPROOT/r13-eng-alias-split"
run_case r13-eng-dot-import     1 "$TMPROOT/r13-eng-dot-import"

echo
echo "=== round 13: the engine detector must not CRY WOLF (§11.4.201(1)) ==="
# Arming a refusal is only fail-closed while it fires on real engines. Each of
# these is legal Go that must keep resolving cleanly; a detector keyed on the
# constructor name alone would refuse all four.
r13_gin r13-eng-negctl-foo <<'R13EOF'
func main() {
	r := gin.New()
	r.Use(AuthMW())
	r.GET("/ok", api.OK)
...
r13_gin r13-eng-negctl-mygin <<'R13EOF'
func main() {
	r := gin.New()
	r.Use(AuthMW())
	r.GET("/ok", api.OK)
...
r13_gin r13-eng-negctl-carrier <<'R13EOF'
func main() {
	r := gin.New()
	r.Use(AuthMW())
	r.GET("/ok", api.OK)
	log.Print("hint: build it with r := gin.Default() and r.Use(AuthMW())")
	r.POST("/wipe", api.WipeHandler)
}
R13EOF

r13_gin r13-eng-negctl-alias-other <<'R13EOF'
import g "github.com/gin-gonic/gin"
...
# The dot-import fixture must assert its LABEL, not merely `exit 1`. Round 13's
# own mutation battery found out why: a mutation that made the engine-regex
# builder RAISE was accepted by an exit-1-only assertion, because Python exits 1
# on an unhandled exception and 1 is this analyzer's FAIL code. The mutation
# survived while the analyzer was in pieces — a §11.4.1 FAIL-bluff living in the
# fixture rather than in the code.
r11_label r13-eng-dot-import-label r13-eng-dot-import \
  'DOT-imported' 'Traceback' 'analyzer failed'

run_case r13-eng-negctl-foo         0 "$TMPROOT/r13-eng-negctl-foo"
run_case r13-eng-negctl-mygin       0 "$TMPROOT/r13-eng-negctl-mygin"
run_case r13-eng-negctl-carrier     0 "$TMPROOT/r13-eng-negctl-carrier"
run_case r13-eng-negctl-alias-other 0 "$TMPROOT/r13-eng-negctl-alias-other"

echo
echo "=== round 13 (BLOCKING sweep): the unmodelled-verb refusal, second member ==="
# The nine GIN_UNMODELLED members are pure refusal-armers — none ever resolves
# into a route — so a split spelling armed nothing and the route vanished
# entirely (exit 0, findings 0, ABSENT from the inventory, hidden behind a
# healthy sibling so the §11.4.201(6) zero-routes net cannot see it either).
# Every member is swept, because fixing one door and leaving eight is exactly
# the mistake that produced this round.
for _v in Any Match Handle Static StaticFile StaticFS StaticFileFS NoRoute NoMethod; do
  d="$TMPROOT/r13-unmod-$_v"; mkdir -p "$d/cmd"
  { printf 'package main\n\nfunc main() {\n\tr := gin.New()\n\tr.Use(AuthMW())\n\tr.GET("/ok", api.OK)\n\tr.\n\t\t%s("/wipe", api.WipeHandler)\n}\n' "$_v"; } > "$d/cmd/main.go"
  mk_policy_go "$d" gin cmd '"AuthMW"' '  - service: svc
    method: GET
    path: /ok
    class: public-by-design
    justification: "Probe."'
  run_case "r13-unmod-$_v" 1 "$d"
done

r13_gin r13-unmod-ws <<'R13EOF'
func main() {
	r := gin.New()
	r.Use(AuthMW())
	r.GET("/ok", api.OK)
	r . Any("/wipe", api.WipeHandler)
}
R13EOF
run_case r13-unmod-ws 1 "$TMPROOT/r13-unmod-ws"

# false-positive guard: a doc string that merely MENTIONS `.Any(` is a CARRIER
# and must not mint a refusal (§11.4.201(7)(a)) — the detector reads `bare`.
r13_gin r13-unmod-negctl <<'R13EOF'
func main() {
	r := gin.New()
	r.Use(AuthMW())
	r.GET("/ok", api.OK)
	log.Print("do not use r . Any( here")
	r.POST("/wipe", api.WipeHandler)
}
R13EOF
run_case r13-unmod-negctl 0 "$TMPROOT/r13-unmod-negctl"

echo
echo "=== round 13 (MINOR-2): a trailing dot is not always a SELECTOR dot ==="
# `x := 3.` is a FLOAT LITERAL and Go's semicolon insertion ends the statement
# after it, so the next line is a new statement — never the far half of a
# selector. The forward walk refused it: a §11.4.201(1) FALSE REFUSAL on legal,
# gofmt-STABLE Go, and by round 9's own doctrine a gate that cries wolf on
# legal code gets bypassed. The two controls prove the fix did not blunt the
# real detector: an identifier may not BEGIN with a digit, so `x2.` is still a
# selector.
r13_gomux r13-float-literal <<'R13EOF'
func NewMux(d *Deps) http.Handler {
	mux := http.NewServeMux()
	mux.HandleFunc("/ok", d.Health.HandleHealth)
	x := 3.
	Handle(x)
	return WithAuth(mux)
}
R13EOF
r13_gomux r13-float-negctl-split <<'R13EOF'
func NewMux(d *Deps) http.Handler {
	mux := http.NewServeMux()
	mux.HandleFunc("/ok", d.Health.HandleHealth)
	open.
...
R13EOF
r13_gomux r13-float-negctl-x2 <<'R13EOF'
func NewMux(d *Deps) http.Handler {
	mux := http.NewServeMux()
	mux.HandleFunc("/ok", d.Health.HandleHealth)
	x2.
...
R13EOF
run_case r13-float-literal      0 "$TMPROOT/r13-float-literal"
run_case r13-float-negctl-split 1 "$TMPROOT/r13-float-negctl-split"
run_case r13-float-negctl-x2    1 "$TMPROOT/r13-float-negctl-x2"

echo
echo "=== round 13 (MINOR-3): RULE D's escape set gains ':' — a TWELFTH shape ==="
# A depth-0 colon separates a CLAUSE HEAD from what follows it, so a `{` after
# one is never the body of a `func(` inside that head. Pre-fix, the func TYPE
# in a one-line `case func(int) int:` anchored a phantom body on the `if`
# brace, the real `return WithAuth(mux)` was excluded as closure-local, and the
# gate exited 1 on legal Go.
r13_gomux r13-case-functype <<'R13EOF'
func NewMux(d *Deps) http.Handler {
	mux := http.NewServeMux()
	mux.HandleFunc("/ok", d.Health.HandleHealth)
	mux.HandleFunc("/wipe", d.Wipe.Handle)
	switch d.K {
	case func(int) int: if d.On { return WithAuth(mux) }
	}
	return nil
}
R13EOF
# the ';' twin, which was ALREADY escaping — proves the new member is the only
# thing that moved.
r13_gomux r13-semi-functype <<'R13EOF'
func NewMux(d *Deps) http.Handler {
	mux := http.NewServeMux()
	mux.HandleFunc("/ok", d.Health.HandleHealth)
	mux.HandleFunc("/wipe", d.Wipe.Handle)
	var _ func(int) int; if d.On { return WithAuth(mux) }
	return nil
}
R13EOF
# false-positive guard: a REAL func literal whose body brace follows a colon in
# a composite-literal KEY must still be seen as a literal, so its inner return
# stays closure-local and cannot prove the wrap.
r13_gomux r13-colon-negctl <<'R13EOF'
func NewMux(d *Deps) http.Handler {
	mux := http.NewServeMux()
	mux.HandleFunc("/ok", d.Health.HandleHealth)
	mux.HandleFunc("/wipe", d.Wipe.Handle)
	m := map[string]func() http.Handler{"a": func() http.Handler { return mux }}
	_ = m
	return WithAuth(mux)
}
R13EOF
run_case r13-case-functype 0 "$TMPROOT/r13-case-functype"
run_case r13-semi-functype 0 "$TMPROOT/r13-semi-functype"
run_case r13-colon-negctl  0 "$TMPROOT/r13-colon-negctl"

echo
echo "=== round 13 (MINOR-1): RULE D's NEWLINE escape is load-bearing ==="
# Round 12 mutation MUT-4 (drop `\n` from the escape set) survived all 156
# assertions: both shipped body fixtures were rescued by an interposed
# `mux := ...` line whose `=` escaped for them. This fixture has NO such rescue
# — the func TYPE is followed DIRECTLY by the `if` whose brace it would hijack,
# and the only `return WithAuth(mux)` lives inside that `if`.
r13_gomux r13-ruleD-newline <<'R13EOF'
func NewMux(d *Deps) http.Handler {
	mux := http.NewServeMux()
	mux.HandleFunc("/ok", d.Health.HandleHealth)
	mux.HandleFunc("/wipe", d.Wipe.Handle)
	var _ func(http.Handler) http.Handler
	if d.On {
		return WithAuth(mux)
	}
	return nil
...
R13EOF
run_case r13-ruleD-newline 0 "$TMPROOT/r13-ruleD-newline"

echo
echo "=== round 13 (NIT-2): the three RULE D/S shapes that had no fixture ==="
# The guide claimed "each is pinned by its own fixture and killed by its own
# mutation". True of the two RULES, FALSE of the shapes: free-function
# func-RESULT, `make(chan func(int))` and `struct{ F func(int) int }` were
# enumerated in the source and pinned by nothing. All three are correct today,
# so these lock the behaviour in rather than reporting a defect.
r13_gomux r13-shape-free-func-result <<'R13EOF'
...
R13EOF
r13_gomux r13-shape-chan-functype <<'R13EOF'
func NewMux(d *Deps) http.Handler {
	mux := http.NewServeMux()
	mux.HandleFunc("/ok", d.Health.HandleHealth)
	mux.HandleFunc("/wipe", d.Wipe.Handle)
...
R13EOF
r13_gomux r13-shape-struct-functype <<'R13EOF'
func NewMux(d *Deps) http.Handler {
	mux := http.NewServeMux()
	mux.HandleFunc("/ok", d.Health.HandleHealth)
	mux.HandleFunc("/wipe", d.Wipe.Handle)
	var s struct{ F func(int) int }
	if d.On {
		_ = s
	}
	return WithAuth(mux)
...
R13EOF
run_case r13-shape-free-func-result 0 "$TMPROOT/r13-shape-free-func-result"
run_case r13-shape-chan-functype    0 "$TMPROOT/r13-shape-chan-functype"
run_case r13-shape-struct-functype  0 "$TMPROOT/r13-shape-struct-functype"

echo
echo "=== round 13 (IMPORTANT-2): the .Group( poison — PINNED, and its bypass closed ==="
# Round 12 mutation MUT-5 (delete the poison loop) survived all 156 assertions:
# the poison was protecting a real property and NOTHING pinned it. And the
# poison itself had a hole — an assignment may break AFTER its `:=`, putting
# the target one line ABOVE the trailing dot, so `GO_ASSIGN_TARGET` (which read
# only the dot line) found nothing and the name was never poisoned. Both
# fixtures below pair the refused declaration with a RESOLVABLE same-named
# group in a sibling function, which is what makes the difference observable:
# unpoisoned, the route inherits that group's prefix AND its credit.
ANALYZER_PY="$REPO_ROOT/scripts/pre_build/lan_route_auth_analyzer.py"
r13_json_lacks() {  # r13_json_lacks <name> <fixture> <needle> <why>
  local name="$1" fx="$2" needle="$3" why="$4" out
  out="$(nice -n 19 python3 "$ANALYZER_PY" --policy "$TMPROOT/$fx/policy.yaml" \
          --root "$TMPROOT/$fx" --json 2>/dev/null || true)"
  if printf '%s' "$out" | grep -q -- "$needle"; then
    FAIL_N=$((FAIL_N + 1))
    printf 'NOT-OK %-22s %s (found %s)\n' "$name" "$why" "$needle" >&2
    printf '%s\n' "$out" | sed 's/^/       | /' >&2
  else
    PASS_N=$((PASS_N + 1)); printf 'ok   %-24s %s\n' "$name" "$why"
  fi
}
r13_json_has() {  # r13_json_has <name> <fixture> <needle> <why>
  local name="$1" fx="$2" needle="$3" why="$4" out
  out="$(nice -n 19 python3 "$ANALYZER_PY" --policy "$TMPROOT/$fx/policy.yaml" \
          --root "$TMPROOT/$fx" --json 2>/dev/null || true)"
  if printf '%s' "$out" | grep -q -- "$needle"; then
    ...
    FAIL_N=$((FAIL_N + 1))
    printf 'NOT-OK %-22s %s (missing %s)\n' "$name" "$why" "$needle" >&2
    printf '%s\n' "$out" | sed 's/^/       | /' >&2
  fi
}

# target ON the dot line — kills MUT-5 (delete the poison loop)
r13_gin r13-group-poison-pin <<'R13EOF'
func other() {
	r := gin.New()
	r.Use(AuthMW())
	g := r.Group("/safe")
...
# target ONE LINE ABOVE the dot — the shipped bypass, closed this round
r13_gin r13-group-bypass <<'R13EOF'
func other() {
	r := gin.New()
	r.Use(AuthMW())
	g := r.Group("/safe")
...
run_case r13-group-poison-pin 1 "$TMPROOT/r13-group-poison-pin"
run_case r13-group-bypass     1 "$TMPROOT/r13-group-bypass"
r13_json_lacks r13-poison-no-prefix r13-group-poison-pin '/safe/wipe' \
  'refused group does not inherit a sibling prefix'
r13_json_lacks r13-bypass-no-prefix r13-group-bypass '/safe/wipe' \
  'target above the dot is poisoned too'
r13_json_has   r13-poison-label     r13-group-poison-pin 'REFUSED to resolve' \
  'the dropped prefix is stated in the route resolution'

echo
echo "=== round 13 (IMPORTANT-1): the mux exclusivity invariant, ACTUALLY pinned ==="
# The analyzer cited "the harness's r11-mux-exclusivity-fuzz needle" for the
# claim that MUX_ANY and MUX_DOTTED_OWNER are mutually exclusive per call site.
# That needle DID NOT EXIST (`grep -c fuzz` returned 0), and round-12 mutation
# MUT-1 — widening MUX_ANY to `\s*\.\s*`, precisely the credit-side widening
# the REFUSE decision exists to prevent — survived all 156 assertions.
# The invariant the `_sites > _named` arithmetic rests on is `named <= sites`.
# This asserts it over a generated adversarial corpus AND proves the instrument
# can SEE a violation by re-running it with the refused widening in place
# (§11.4.201(7)(b) control needle: a null is not evidence until the same
# instrument, on the same path, returns non-null on a known-present needle).
fuzz_out="$(nice -n 19 timeout 120 python3 - "$ANALYZER_PY" <<'PYEOF'
import importlib.util, random, re, sys
spec = importlib.util.spec_from_file_location("lra", sys.argv[1])
m = importlib.util.module_from_spec(spec)
spec.loader.exec_module(m)

random.seed(0xB0B102)
ATOMS = [
    "mux.", "mux .", "mux. ", "mux . ", "s.mux.", "s.mux .", "d.Health.",
    "HandleFunc(", "Handle(", " HandleFunc(", "\tHandle(",
    '"/a", h)', '"/b", d.H.Handle)', ", h)", ")", ";", " ",
    "http.", "newServeMux().", "muxes[0].", "Server{}.mux.",
    "// mux.HandleFunc(", '"mux.HandleFunc("', "return ", "if x {", "}",
    "WithAuth(", "func(", "w, r)", "_ = ", "x :=", "\t",
]
def gen():
    return "".join(random.choice(ATOMS) for _ in range(random.randint(1, 7)))
...
def scan(any_re):
    ...
    for _ in range(25000):
        line = gen()
        sites = len(m.MUX_CALL.findall(line))
        named = len(any_re.findall(line)) + len(m.MUX_DOTTED_OWNER.findall(line))
        if named > sites:
            bad += 1
    return bad

shipped = scan(m.MUX_ANY)
# CONTROL NEEDLE 1 — strip the carrier lookbehind.
needle1 = scan(re.compile(r"(\w+)\.(?:HandleFunc|Handle)\s*\("))
# CONTROL NEEDLE 2 — the REFUSED widening (round-12 MUT-1) itself.
needle2 = scan(re.compile(r"(?<![\w.])(\w+)\s*\.\s*(?:HandleFunc|Handle)\s*\("))
print(f"{shipped} {needle1} {needle2}")
PYEOF
)" || fuzz_out="ERR"
set -- $fuzz_out
if [[ "${1:-x}" == "0" && "${2:-0}" -gt 0 && "${3:-0}" -gt 0 ]]; then
  PASS_N=$((PASS_N + 1))
  printf 'ok   %-24s named<=sites over 25000 lines (0 violations; needles %s/%s SEE)\n' \
    r11-mux-exclusivity-fuzz "$2" "$3"
else
  FAIL_N=$((FAIL_N + 1))
  printf 'NOT-OK %-22s shipped=%s needle1=%s needle2=%s (want 0 / >0 / >0)\n' \
    r11-mux-exclusivity-fuzz "${1:-?}" "${2:-?}" "${3:-?}" >&2
fi

echo
echo "=== round 13 (§11.4.1): an analyzer CRASH is an ERROR, never a FINDING ==="
# Python exits 1 on an unhandled exception, and 1 is this analyzer's FAIL code,
# so before this round ANY internal defect surfaced through the wrapper as
# "FAIL: blocking finding(s)" with no finding to show — the reader sent to fix a
# route that is not broken. This runs a COPY of the analyzer with a deliberate
# exception injected (the repo file is never touched) and requires exit 2, the
# wrapper's ERROR class. The second half is the control needle: the SAME copy,
# uninjected, must still exit 0 on the same fixture — otherwise a "2" would
# prove nothing about the injection (§11.4.201(7)(b)).
crash_py="$TMPROOT/crash_analyzer.py"
nice -n 19 python3 - "$ANALYZER_PY" "$crash_py" <<'CRASHPY'
import sys
src, dst = sys.argv[1], sys.argv[2]
s = open(src).read()
marker = "    ap = argparse.ArgumentParser"
assert s.count(marker) == 1, "injection point not unique"
s = s.replace(marker, '    raise RuntimeError("injected meta-test crash")' + chr(10) + marker, 1)
open(dst, "w").write(s)
CRASHPY
ctl_py="$TMPROOT/ctl_analyzer.py"; cp "$ANALYZER_PY" "$ctl_py"
crash_rc=0
nice -n 19 timeout 60 python3 "$crash_py" --policy "$TMPROOT/r13-unmod-negctl/policy.yaml"   --root "$TMPROOT/r13-unmod-negctl" >/dev/null 2>&1 || crash_rc=$?
ctl_rc=0
nice -n 19 timeout 60 python3 "$ctl_py" --policy "$TMPROOT/r13-unmod-negctl/policy.yaml"   --root "$TMPROOT/r13-unmod-negctl" >/dev/null 2>&1 || ctl_rc=$?
if [[ "$crash_rc" == 2 && "$ctl_rc" == 0 ]]; then
  PASS_N=$((PASS_N + 1))
  printf 'ok   %-24s crash=2 (ERROR) / uninjected control=0
' r13-crash-is-error
else
  FAIL_N=$((FAIL_N + 1))
  printf 'NOT-OK %-22s crash_rc=%s (want 2) control_rc=%s (want 0)\n' \
    r13-crash-is-error "$crash_rc" "$ctl_rc" >&2
fi
