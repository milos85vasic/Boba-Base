...
- [ ] T042 Run the full **50**-invariant gate via `bash scripts/commit-push-all.sh` with NO `BOBA_SYNC_SKIP_CI` (count corrected 2026-08-25 from a stale "44+1": measured 41 bracket-form + 9 `run_const_gate` labels, all denominators `/50`, single value, on a two-form control-needled instrument — a bracket-only scan sees 41 and under-reports, which is the §11.4.201(6) false-null that bit two separate audits of this file) — this change touches executable surfaces, so the long gate must actually run [SC-004, SC-005]
