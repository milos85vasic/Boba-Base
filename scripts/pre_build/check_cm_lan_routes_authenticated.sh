...
#   2 — ERROR (usage error, missing analyzer/policy, unusable interpreter, or
#       an analyzer CRASH). Round 13: an unhandled exception used to exit 1,
#       which is this gate's FAIL code, so an internal defect was reported as
#       "blocking finding(s)" with no finding to show — a §11.4.1 FAIL-bluff
#       that sent the reader to fix a route that was not broken. The analyzer
#       now traps and exits 2, so a crash is never mistaken for a verdict
#       about a route.
#
# Cross-references:
...
# The range ends at the Guide line of the Cross-references block; round 13
# added six lines to the Verdict block, and leaving 95 here would have silently
# truncated --help mid-sentence.
print_help() { sed -n '2,101p' "${BASH_SOURCE[0]}"; }
