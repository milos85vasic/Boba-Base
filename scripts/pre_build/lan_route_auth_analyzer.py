...
# ROUND 13 (BLOCKING sweep) — THE SECOND MEMBER OF THE SAME CLASS.
# These nine are pure REFUSAL-ARMERS: none of them ever resolves into a route,
# so their whole contribution is the "this gin registration form is not
# modelled" finding. The same-line detector is `\w\.<verb>\s*\(`, which
# requires the receiver, the dot and the member to be ADJACENT — so a split or
# whitespace-separated spelling armed nothing, the line fell through to
# `if not GIN_VERB.search(bln): continue`, and the route vanished. Measured
# 2026-08-27 across ALL NINE members beside a healthy sibling route, every one
# BYTE-IDENTICAL through gofmt:
#     r.
#         Any("/wipe", api.WipeHandler)
#   -> exit 0, routes 1, findings 0, `/wipe` ABSENT from the inventory — the
#      §11.4.201(6) zero-routes net cannot see it either, because the sibling
#      keeps the file non-empty. The one-line control correctly exits 1.
# `r.Any ("/wipe", …)` was already caught (`\s*\(` tolerated it); the two open
# axes were whitespace TOUCHING THE DOT and the newline split, and both are
# closed below by the SAME shared factories the verbs and `.Group(` use, so the
# three halves of one primitive cannot drift apart again (§11.4.250).
# Longest-first so `StaticFileFS` is never shadowed by `Static`.
_GIN_UNMOD_ALT = "|".join(sorted(GIN_UNMODELLED, key=len, reverse=True))
...
#   in a SIGNATURE  — the METHOD RECEIVER CLAUSE (the one shape round 9 fixed,
#                     listed here so the count and the enumeration agree —
#                     round 12 NIT-1 found the prose claiming ELEVEN above a
#                     list of ten), func-typed PARAM (method AND free
#                     function), func-typed RESULT (method AND free function),
#                     variadic `...func(*Server)`, named result
#                     `(h func(int) int, e error)`
...
def _body_brace(text, start):
    ...
    ROUND 13 (MINOR-3) — `:` JOINS THE ESCAPE SET, a TWELFTH shape.
    A depth-0 colon separates a CLAUSE HEAD from the statements that follow it
    (`case func(int) int:`, `default:`, a `Loop:` label), so a `{` after one
    can never be the body of a `func(` inside that head. Measured 2026-08-27
    on the legal one-line `case func(int) int: if d.On { return WithAuth(mux) }`
    (`gofmt -e` exit 0): the case clause's func TYPE anchored a phantom body on
    the `if` brace, the real `return WithAuth(mux)` was excluded as
    closure-local, and the gate exited 1 claiming "mux 'mux' is returned
    WITHOUT an auth marker" — a §11.4.201(1) FALSE REFUSAL on legal Go, the
    same cry-wolf class the eleven shapes above were fixed for.
    `:=` was ALREADY escaping on its `=`, so this widens nothing that was
    previously resolvable: the only depth-0 colons it newly reaches are clause
    heads and labels, both of which end a construct rather than continue one.
    """
...
#
# HONEST SCOPE (§11.4.6), CORRECTED IN ROUND 13 — the sentence that stood here
# was a FALSE ABSOLUTE in two places and the audit that wrote it never ran the
# cases. It claimed `.Use(`, `gin.Default()` and `http.NewServeMux()` alike
# "each DROP THE CREDIT and are therefore fail-CLOSED and LOUD". Measured
# 2026-08-27, one construct at a time:
#
#   `.Use(`               TRUE. GIN_USE stops matching, the credit is not
#                         granted, the route reads uncovered and the gate
#                         exits 1 naming it. Verified in all three spellings.
#   `http.NewServeMux()`  TRUE. MUX_DECL stops matching, the owner is not a
#                         resolvable mux, and the route is reported
#                         "fail-closed: treated as unwrapped", exit 1.
#   `gin.New/Default()`   **FALSE** — and this was the round-12 BLOCKING. An
#                         engine DECLARATION carries NO CREDIT TO DROP: it
#                         feeds only the `len(engines) > 1` and "bound N times"
#                         REFUSALS, while `GIN_USE` grants credit
#                         independently and file-scoped. Making it invisible
#                         therefore DISARMS the refusal that is the
#                         precondition for file-scoped crediting, and the gate
#                         exits 0 over an unauthenticated mutating route.
#
# THE CLASS, not the door (§11.4.250): a construct whose ABSENCE DROPS A CREDIT
# degrades fail-closed and loud; a construct whose ABSENCE DISARMS A REFUSAL
# degrades fail-OPEN and silent. Round 13 swept every refusal-arming construct
# in both Go extractors for this property and found TWO open —
# `gin.New/Default()` above and the nine `GIN_UNMODELLED` members — and both
# are now armed in every legal spelling. The constructs measured CLOSED by that
# same sweep, recorded so the next round does not re-derive them:
# `GIN_DOTTED_OWNER`, `MUX_DOTTED_OWNER`, `MUX_CALL`/`MUX_ANY`, the
# `owner == "http"` DefaultServeMux rule, and the `;`/multi-statement rule.
# `.Group(` is refused alongside the verbs for the separate reason that it
# corrupts the route PATH and can be blessed by a stale exemption.
...
# ROUND 13 (MINOR-2) — A TRAILING DOT IS NOT ALWAYS A SELECTOR DOT.
# `x := 3.` is a FLOAT LITERAL (3.0), not a half-written selector, and Go's
# semicolon insertion ENDS the statement after it — so the next line is a new
# statement, never the far half of a selector. The forward walk read the two as
# one split selector and refused legal, gofmt-STABLE Go:
#     x := 3.
#     Handle(x)
# measured 2026-08-27 as exit 1 "a .HandleFunc(/.Handle( registration whose
# receiver is separated from the member by whitespace or a newline" — a
# §11.4.201(1) FALSE REFUSAL, and by round 9's own doctrine a gate that cries
# wolf on legal code gets bypassed, which is how the silent-drop class returns.
#
# THE DISCRIMINATOR IS EXACT, not a heuristic: a Go identifier may never BEGIN
# with a digit (spec, "Identifiers"), so a `\w` run ending at the dot whose
# FIRST character is a digit cannot be an operand of a selector — it is a
# numeric literal. `x2.` stays a selector (first char `x`), `_1.` stays a
# selector, `foo[3].` stays a selector (no `\w` run touches the dot), and `3.`
# / `1_000.` / `0x1F.` are numbers. Nothing that WAS refused stops being
# refused except this one provably-non-selector class.
GO_NUM_BEFORE_DOT = re.compile(r"(\w+)\.\s*$")
...
# ROUND 13 (IMPORTANT-2) — THE POISON READ ONLY THE DOT LINE.
# A Go assignment may break AFTER its `:=`, which puts the target one or more
# lines ABOVE the trailing selector dot the refusal fires on:
#     g :=
#         parent.
#         Group("/admin")
# `GO_ASSIGN_TARGET.findall(<the dot line>)` sees `parent.` — no `=`, no
# target — so the refusal fired and the POISON MISSED. Measured 2026-08-27 with
# a resolvable same-named group in a sibling function: exit 1 (the refusal is
# there) but the inventory reported `POST /safe/wipe auth_wired=TRUE`, the
# prefix AND the credit both inherited from a binding that has nothing to do
# with this registration — the shipped in-source claim "the name is poisoned so
# no later registration can inherit a prefix that was never resolved" was
# simply untrue of this spelling.
# The walk-back is deliberately ONE STEP and gated on the previous code line
# ENDING in an assignment operator, which is the only shape that can continue
# an assignment onto the dot line. Over-poisoning is the fail-CLOSED direction
# (a poisoned name loses its prefix and its credit, never gains either), so an
# imprecise hit costs a loud refusal, never a silent pass.
GO_ASSIGN_CONT = re.compile(r":?=\s*$")


def _assign_targets_at(bare_lines, idx):
    """Assignment targets governing the statement that reaches line ``idx``."""
    here = GO_ASSIGN_TARGET.findall(bare_lines[idx])
    if here:
        return here
    for k in range(idx - 1, -1, -1):
        if not bare_lines[k].strip():
            continue
        if GO_ASSIGN_CONT.search(bare_lines[k]):
            return GO_ASSIGN_TARGET.findall(bare_lines[k])
        return []
    return []
...
def _selector_is_split(bare_lines, idx, head_re):
    ...
    if not GO_TRAILING_DOT.search(bare_lines[idx]):
        return False
    # A numeric literal's dot ends a statement (§ROUND 13 / MINOR-2 above), so
    # the next line cannot be the far half of a selector.
    _tok = GO_NUM_BEFORE_DOT.search(bare_lines[idx])
    if _tok and _tok.group(1)[0].isdigit():
        return False
...
# ---- ROUND 13 (BLOCKING) — THE INVERTED PRIMITIVE ------------------------
# Round 11 closed "a selector can split at a dot" for every construct that
# GRANTS CREDIT or NAMES A ROUTE, and its HONEST SCOPE block then claimed the
# same primitive merely "DROPS THE CREDIT and is therefore fail-CLOSED and
# LOUD" for `gin.Default()`. THAT SENTENCE WAS FALSE, and the audit that wrote
# it never ran the case: an engine DECLARATION carries no credit to drop.
# `GIN_ENGINE` feeds ONLY the `len(engines) > 1` refusal and the "bound N
# times" refusal; `uses` (`GIN_USE`) grants credit INDEPENDENTLY, file-scoped,
# keyed on the bare name. So making the declaration invisible drops nothing —
# it DISARMS THE REFUSAL that is the load-bearing precondition for file-scoped
# crediting (the exact precondition `gin-one-engine-two-funcs` pins).
#
# THE REAL PRIMITIVE, stated so the next round does not have to rediscover it:
#   SILENCE IS SAFE FOR A CREDIT-CARRIER AND UNSAFE FOR A REFUSAL-ARMER.
# A construct whose ABSENCE drops a credit degrades fail-closed and loud. A
# construct whose ABSENCE disarms a refusal degrades fail-OPEN and SILENT. The
# two look identical in the source and are opposite in consequence, so every
# refusal-ARMING construct must be detectable in EVERY legal spelling.
#
# Measured 2026-08-27, `gofmt -e` accepting all nine and TWO of them
# BYTE-IDENTICAL through gofmt (the dot-newline pair):
#     func buildOps() { r := gin.New(); r.Use(AuthMW()); r.GET("/ok", …) }
#     func main()     { r := gin.
#                              Default()
#                       r.POST("/wipe", …) }
#   -> POST /wipe auth_wired: TRUE, resolution "owner 'r' carries .Use(marker)
#      at or above this line", findings [], exit 0 — `main()`'s engine is a
#      FRESH `gin.Default()` carrying NO middleware, and the route is credited
#      from an unrelated same-named engine in another function.
#   CONTROL A (both spelled `gin.Default()`): the `2 gin engine declarations in
#      one file` refusal FIRES, exit 1 — proving the split spelling is
#      precisely what disarms it, not some unrelated difference.
#   CONTROL B (`r := gin . Default()`, one line): same false-safe, exit 0.
# Every same-line whitespace position measured open: `gin . New()`,
# `gin .New()`, `gin. New()`, `gin.New ()`, `gin.New( )`, `gin.\tDefault()`.
#
# THE FIX IS TO ARM, NOT TO REFUSE SEPARATELY. Adding an engine SITE can only
# make the gate refuse MORE (it feeds two refusal counters and grants nothing),
# so widening here is strictly fail-closed and cannot mint a false PASS. The
# split spelling therefore becomes BYTE-FOR-BYTE Control A rather than growing
# a second finding class for a hole Control A already reports.
GIN_IMPORT_PATH = "github.com/gin-gonic/gin"
# `import g "…/gin"` binds the package to `g`, so a hard-coded `gin.` selector
# is blind to it — the SAME absence-disarms class on the naming axis rather
# than the whitespace axis (measured 2026-08-27: `r := g.Default()` beside a
# `.Use`-carrying sibling engine → exit 0, POST /wipe auth_wired TRUE,
# gofmt-STABLE). Resolved from the file's OWN import block, so nothing is
# guessed and no other package's `.New()` is ever mistaken for an engine.
GO_IMPORT_LINE = re.compile(r'^\s*(?:import\s+)?(?:(\w+|\.)\s+)?"([^"]+)"\s*$')


def _pkg_local_name(code_lines, import_path, default):
    """The local name this file binds ``import_path`` to.

    Returns ``default`` when the file does not import it (or imports it
    plainly), the alias when one is given, and ``None`` for a DOT import
    (`import . "…"`), where the constructor is called with no selector at all
    and this model has nothing to key on.
    """
    for ln in code_lines:
        m = GO_IMPORT_LINE.match(ln)
        if not m or m.group(2) != import_path:
            continue
        alias = m.group(1)
        if alias == ".":
            return None
...
def _gin_engine_res(pkg):
    """`(same-line, split-half)` engine-declaration regexes for a file whose
    gin package is bound to ``pkg``.

    Whitespace-tolerant at EVERY position the round-6 spelling was rigid. The
    `=` anchors the package position, so no lookbehind is needed: on
    `r := mygin.New()` the only `=` forces the package to start at `m`, which
    cannot match `gin`. The bare-name capture is deliberately IDENTICAL to the
    round-6 form (no leading lookbehind) so a dotted assignment target keeps
    counting as a site exactly as it did before — this round widens spellings,
    it does not re-open the target question.
    """
    p = re.escape(pkg)
    return (
        re.compile(r"(\w+)\s*:?=\s*" + p + r"\s*\.\s*(?:New|Default)\s*\(\s*\)"),
        re.compile(r"(\w+)\s*:?=\s*" + p + r"\s*\.\s*$"),
    )


# The far half of a `<pkg>.`-terminated engine declaration. Keyed on the
# CONSTRUCTOR name alone, and only ever consulted together with the near-half
# regex above, so `x := foo.` + `New()` can never mint an engine (measured as
# a golden-FALSE control: it must stay exit 0).
GIN_UNMOD_HEAD = _selector_head(_GIN_UNMOD_ALT)
GIN_UNMOD_WS = _ws_selector(_GIN_UNMOD_ALT)
GIN_ENGINE_HEAD = _selector_head("New|Default")
GIN_ENGINE = re.compile(r"(\w+)\s*:?=\s*gin\s*\.\s*(?:New|Default)\s*\(\s*\)")
...
def extract_gin(root, svc, markers):
    ...
    for p in _walk_files(root, svc, ".go", skip_test=True):
        ...
        # ROUND 13 (BLOCKING) — the engine detector is built from THIS FILE's
        # own gin import, so an aliased import is seen and no other package's
        # `.New()` is ever mistaken for an engine.
        ...
        if _gin_pkg is None:
            unmodelled.append(_unmod(
                ...
                "middleware crediting cannot be armed, so the file is refused "
                "rather than credited blind"))
            continue
        _ENG_RE, _ENG_SPLIT = _gin_engine_res(_gin_pkg)
        ...
        for i, (ln, bln) in enumerate(zip(code, bare), 1):
            ...
            # silently disarming them. Both halves are required for the split
            # form: this line must be `<target> := <pkg>.` AND the next line
            # carrying code must OPEN with `New(`/`Default(` — so
            # `x := foo.` + `New()` and `x := mygin.` + `Default()` stay
            # untouched (§11.4.201(1) golden-FALSE controls).
            _eng_here = list(_ENG_RE.findall(bln))
            if _selector_is_split(bare, i - 1, GIN_ENGINE_HEAD):
                _eng_here += _ENG_SPLIT.findall(bln)
            for var in _eng_here:
            ...
            if (GIN_GROUP_WS_SELECTOR.search(bln)
                ...
                for _v in _assign_targets_at(bare, i - 1):
        ...
        for i, (ln, bln) in enumerate(zip(code, bare), 1):
            ...
            # ROUND 13 (BLOCKING sweep) — the split and whitespace halves of
            # the same nine members. Detected on `bare` like the same-line
            # rule above, so a doc string that merely MENTIONS `.Any(` still
            # cannot mint a refusal (§11.4.201(1)). No line can reach both this
            # rule and the one above: that one needs `\w` immediately before
            # the dot, these two need whitespace or a newline there.
            if (GIN_UNMOD_WS.search(bln)
                    or _selector_is_split(bare, i - 1, GIN_UNMOD_HEAD)):
                unmodelled.append(_unmod(
                    svc["id"], f"{rel}:{i}", ln.strip()[:70],
                    "an unmodelled gin registration form (.Any/.Match/.Handle/"
                    ".Static*/.NoRoute/.NoMethod) whose receiver is separated "
                    "from the member by whitespace or a newline; the route it "
                    "registers is not resolvable and dropping the line would "
                    "hide it from the inventory entirely"))
            ...
            for owner, meth, path in GIN_ROUTE.findall(ln):
                ok = covered(owner, i)
                # ROUND 13 (IMPORTANT-2) — SAY SO WHEN THE PREFIX WAS DROPPED.
                # A name in `unresolvable_groups` resolves at an UNPREFIXED
                # path and with no inherited middleware, and the route said
                # only "no .Use(marker) covers owner 'g'" — true, and silent
                # about the load-bearing half, so a reader comparing the
                # inventory against the source sees a path that is not the
                # one gin serves and is told nothing about why. That is the
                # wrong/absent-label class (F8) this engine has now met four
                # times. The VERDICT is unchanged — every such file already
                # exits 1 on its own refusal — this makes the inventory stop
                # under-explaining itself.
                _dropped = owner in unresolvable_groups
                _why = (
                ...
                if _dropped:
                    _why += (
                        f"; '{owner}' is a router group this model REFUSED to "
                        f"resolve (see the FINDING above), so this path is "
                        f"reported UNPREFIXED and inherits nothing — the path "
                        f"gin actually serves is the group's prefix plus this "
                        f"one")
                routes.append(Route(
...
if __name__ == "__main__":
    # ROUND 13 (§11.4.1) — A CRASH MUST NOT READ AS A ROUTE FINDING.
    # Python exits 1 on an unhandled exception, and 1 is this analyzer's FAIL
    # code — so any internal defect (a TypeError, a bad regex build, an
    # unreadable file) came back through the wrapper as
    # "FAIL: blocking finding(s)" with no finding to show. That is a FAIL-bluff
    # in the §11.4.1 sense: the gate fails for a script-internal reason and
    # reports it as a product defect, sending the reader to fix a route that is
    # not broken. Found by round 13's OWN mutation battery — a mutation that
    # made the engine-regex builder raise was accepted by a fixture asserting
    # only `exit 1`, so the mutation survived while the analyzer was in pieces.
    # Exit 2 is the wrapper's ERROR class, which prints "analyzer failed" and is
    # never mistaken for a verdict about a route.
    try:
        sys.exit(main())
    except SystemExit:
        raise
        ...
        import traceback
        traceback.print_exc()
        print("ERROR: the analyzer crashed. This is an ERROR (exit 2), NOT a "
              "route finding — no verdict about any route was reached "
              "(§11.4.1).", file=sys.stderr)
        sys.exit(2)
