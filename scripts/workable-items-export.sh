#!/usr/bin/env bash
# workable-items-export.sh — Regenerate all docs from the workable-items DB,
# produce HTML/PDF/DOCX siblings, and stub any missing domain-summary pages.
#
# RENAMED 2026-08-15 (BOB-104): this script was previously named `docs_chain.sh`
# — that name was a MISNOMER: this script is a docs-REGENERATION wrapper
# (workable-items export + markdown export + summary stubs), NOT the §11.4.106
# canonical Docs Chain engine. The REAL Docs Chain engine — the mechanical
# doc/DB sync enforcer per §11.4.106 — is now incorporated at
# `constitution/submodules/docs_chain/` (`git@github.com:vasic-digital/docs_chain.git`,
# pinned to helixcode-v1.1.0). Invoke the real engine via:
#     ./constitution/submodules/docs_chain/docs_chain {doctor|verify|sync} --all
# against contexts under `.docs_chain/contexts/*.yaml`.
#
# Usage:
...
#               in constitution/submodules/docs_chain/; this script's Step 1
#               feeds §11.4.106 by regenerating the .md sources the engine
#               then propagates to derived .html/.pdf/.docx).
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            echo "Usage: bash scripts/workable-items-export.sh [--check-only]"
            echo
...
> Auto-generated by workable-items-export.sh — §11.4.106 procedure docs stub.
...
---
*This stub was created by \`scripts/workable-items-export.sh\`. Replace with actual domain documentation as part of §11.4.106 Phase 4+.*
SUMMARYEOF
    stubbed=$((stubbed + 1))
}
