...
echo "[1/56] constitution/ directory exists"
...
echo "[2/56] constitution/Constitution.md §11.4 anchor"
...
echo "[3/56] constitution/CLAUDE.md anti-bluff covenant anchor"
...
echo "[4/56] constitution/AGENTS.md anti-bluff covenant anchor"
...
echo "[5/56] Parent CLAUDE.md inheritance pointer"
...
echo "[6/56] Parent AGENTS.md inheritance pointer"
...
echo "[7/56] Parent CONSTITUTION.md inheritance pointer"
...
echo "[8/56] Parent CLAUDE.md §11.4 propagation anchor"
...
echo "[9/56] Parent AGENTS.md §11.4 propagation anchor"
...
echo "[10/56] Parent CONSTITUTION.md §11.4 propagation anchor"
# Structural, per §11.4.201(7)(a) — see invariant 8's block comment for the
# measured declaration shapes and the §11.4.109 prefix-collision guard.
...
echo "[11/56] .claude/settings.json with PreToolUse guard hook"
...
echo "[12/56] docs/AGENT_GUARDRAILS.md SUBAGENT CONSTITUTIONAL PREAMBLE"
...
echo "[13/56] docs/AGENT_GUARDRAILS.md ORCHESTRATOR PRE-ACTION CHECKLIST"
if [[ -f "${GUARDRAILS_FILE}" ]] && \
...
echo "[14/56] constitution/scripts/hooks/guard-forbidden-commands.sh"
...
echo "[15/56] tests/hooks/test_guard_forbidden_commands.sh"
...
echo "[16/56] CM-MARKDOWN-EXPORT-SYNC: all-Markdown export freshness (§11.4.65)"
...
echo "[17/56] CM-WORKABLE-ITEMS-VALIDATE: workable-items validate (§11.4.93/§11.4.95)"
...
echo "[18/56] CM-WORKABLE-ITEMS-EXPORT-VALIDATE: workable-items-export.sh --check-only (§11.4.93/§11.4.65)"
...
echo "[19/56] CM-QA-DISCOVERY-LEDGER-FRESH: ledger fresh + counts aligned (§11.4.238)"
...
echo "[20/56] CM-QA-IS-THE-DISCOVERER: every out-of-band entry carries required fields (§11.4.238(C))"
if [[ ! -f "${LEDGER}" ]]; then
...
echo "[21/56] CM-COVENANT-114-238-PROPAGATION: §11.4.238 propagates (§11.4.227(B))"
PROP_FINDINGS=""
...
echo "[22/56] CM-WORKABLE-ITEMS-EXPORT-STEP1-REAL-INVOCATION: export step 1 really invoked (§11.4.238 RD2-41a)"
if [[ ! -f "${DOCS_CHAIN}" || ! -x "${DOCS_CHAIN}" ]]; then
...
echo "[23/56] CM-NO-PRODUCTION-MUTATION-RESIDUE: no mutation-marker residue in production sources (§11.4.84)"
...
echo "[24/56] CM-DOCS-CHAIN-ENGINE-VERIFY: docs_chain engine verify --all (§11.4.106)"
...
echo "[25/56] CM-RESOURCE-PRESSURE-SIGNATURE-CHECK: proactive host-pressure probe (§12.12, task #77/BOB-076)"
...
# itself (§11.4.89).
echo "[26/56] CM-BADGE-FRESHNESS-CHECK: README badges match live counts (§11.4.259, BOB-118)"
...
echo "[27/56] CM-KILLPG-PGID-GUARD: no unguarded process-group kill calls (§11.4.263, BOB-126)"
...
echo "[28/56] CM-TEST-MOCK-PID-EXPLICIT-INT: no unguarded subprocess-mock pid in tests (§11.4.263, BOB-126)"
...
# signal.
echo "[29/56] CM-TEST-MOCK-PID-PATCHED-WHEN-REAL-PID: no unpatched real-pid subprocess-mock in tests (§11.4.263, BOB-127)"
...
echo "[31/56] CM-COVENANT-PROPAGATION-BLOCK-INTEGRITY: 17 pack-unbound anchors (§11.4.227(B))"
...
echo "[32/56] CM-COVENANT-PROPAGATION-SUITE: 30 pack-driven anchors (§11.4.227(B), ADVISORY)"
...
echo "[39/56] CM-DANGEROUS-COMBINATION-FAIL-CLOSED: fail-open scan over first-party source (§11.4.252, ADVISORY)"
...
echo "[41/56] CM-OPENDESIGN-UI-SYSTEM: Angular frontend design-system audit (§11.4.162, ADVISORY)"
...
echo "[44/56] CM-HEALTHCHECK-COVERS-SERVED-PORTS: healthchecks probe every served port (§11.4.254, BOB-138)"
...
# BLOCKING.
echo "[45/56] CM-OWNERSHIP-INVARIANTS: operator-owned writes not reverted (§11.4.201, FR-011)"
...
# BLOCKING.
echo "[46/56] CM-PLUGIN-COUNT: documented plugin counts match their derivation (BOB-149)"
...
# BLOCKING.
echo "[47/56] CM-GO-TOOLCHAIN-MATCHES-BUILDER: Dockerfile builder satisfies go.mod (BOB-153)"
...
echo "[48/56] CM-RUNTIME-DEPS-PARITY: test stack and production run the same deps (BOB-154)"
...
# ---------------------------------------------------------------------------
echo "[49/56] CM-CLOSURE-SEAM-BINDS: done-but-open rows found mechanically (§11.4.226)"
...
# author's own set).
echo "[50/56] CM-EXPORT-CHARSET-VALID: exports declare a charset AND the ratchet is current (§11.4.238)"
...
echo "[51/56] CM-SERVED-BUNDLE-FRESH: the served dashboard bundle matches its sources (BOB-183)"
...
echo "[52/56] CM-WORKABLE-ITEMS-BINARY-FRESH: the shipped workable-items binary matches its sources (BOB-188)"
...
echo "[53/56] CM-QBITTORRENT-WEBUI-CREDENTIALS: WebUI credentials enforced and auth not bypassed (§11.4.239)"
...
echo "[54/56] CM-NO-TEST-TAG-DEBRIS: no test-generated tags in the live qBittorrent (§11.4.14)"
...
echo "[55/56] CM-NO-UNSCOPED-LIVE-DESTRUCTION: test-tree deletes are diff-scoped (§9 / §11.4.14)"
...
echo "[56/56] CM-GITIGNORE-SWALLOW-GUARD: no first-party source file is silently swallowed by .gitignore (§11.4.201(6), BOB-212)"
