...
#  51. CM-SERVED-BUNDLE-FRESH: runs scripts/pre_build/check_cm_served_bundle_fresh.sh
#      --if-present (BOB-183) — the compiled Angular bundle users actually load
#      is gitignored, so a divergence from frontend/src never shows in a diff.
#      install.sh:133 asserted only that the dist DIRECTORY EXISTED, so the
#      BOB-164 contrast fix shipped GREEN at the source layer while the served
#      artifact still carried the defect (§11.4.108 layer 2). Oracle is a CONTENT
#      FINGERPRINT (sha256 over the sorted build-input set, §11.4.86), never
#      mtime — mtime moves on checkout/touch/copy without a byte changing and
#      would be a §11.4.201(1) false-positive refusal.
#      NON-BLOCKING on absence at this PRE-build seam (§11.4.69
#      artifact_not_yet_built); BLOCKING on a present-but-stale bundle and on a
#      BLIND input scan. The POST-build seam (install.sh) refuses on absence too.
...
#  52. CM-WORKABLE-ITEMS-BINARY-FRESH: runs scripts/pre_build/check_cm_workable_items_binary_fresh.sh — the SHIPPED,
#      git-tracked workable-items binary must match its Go sources. Invariant 17
#      no longer uses it (it builds from source every run, BOB-188); this stays
#      as defence-in-depth for the other callers of the shipped binary.
#  57. CM-NO-FAIL-OPEN-SKIP: runs scripts/pre_build/check_cm_no_fail_open_skip.sh —
#      no test may turn evidence the far side ANSWERED (a response status, an
#      empty body, an except that also catches answered HTTP errors) into a
#      SKIP (§11.4.69, BOB-161). Finding SET vs a monotone-decreasing baseline
#      (NEW or STALE row -> FAIL); rc=2 (blind/unparseable) is UNVERIFIED -> FAIL.
#  60. CM-PREBUILD-INVOKED-EXECUTABLES-TRACKED: runs
#      scripts/pre_build/check_prebuild_invoked_executables_tracked.sh (BOB-231)
#      — every executable this sweep invokes, and the helpers those scripts call,
#      must be git-tracked. BLOCKING; untracked tests/**/test_*.sh are ADVISORY.
#  61. CM-SCRIPT-DOCS-SYNC (§11.4.18, BOB-242): scripts/pre_build/check_cm_script_docs_sync.sh
#      — every scripts/**/*.sh has an in-sync docs/scripts/<name>.md; ratchet
#      baseline scripts/pre_build/cm_script_docs_sync.baseline. BLOCKING.
#  62. CM-DOC-REVISION-HEADER-PRESENT (§11.4.44, BOB-242):
#      scripts/pre_build/check_cm_doc_revision_header_present.sh — every
#      docs/scripts/*.md carries Revision + UTC Last modified. BLOCKING.
...
echo "[1/62] constitution/ directory exists"
...
echo "[2/62] constitution/Constitution.md §11.4 anchor"
...
echo "[3/62] constitution/CLAUDE.md anti-bluff covenant anchor"
...
echo "[4/62] constitution/AGENTS.md anti-bluff covenant anchor"
...
echo "[5/62] Parent CLAUDE.md inheritance pointer"
...
echo "[6/62] Parent AGENTS.md inheritance pointer"
...
echo "[7/62] Parent CONSTITUTION.md inheritance pointer"
...
echo "[8/62] Parent CLAUDE.md §11.4 propagation anchor"
...
# quoted example — so the check could pass on a file carrying no propagated-
# clauses declaration at all. The real structure (read 2026-09-01) is a single
# line-start **Propagated clauses** declaration that ENUMERATES the clause:
#   CLAUDE.md       `**Propagated clauses** (Helix Universal Constitution): §11.4.10 (…), …`
#   AGENTS.md       `> **Propagated clauses:** §11.4.10, …`
#   CONSTITUTION.md `> **Propagated clauses (Helix Universal Constitution):** §11.4.10, …`
# The optional '>' allows the blockquote form; the trailing ([^0-9]|$) guard
# stops §11.4.109 — which is enumerated on the SAME line — from satisfying a
# check for §11.4.10 by prefix.
if grep -qE '^>?[[:space:]]*\*\*Propagated clauses.*§11\.4\.10([^0-9]|$)' "${PROJECT_ROOT}/CLAUDE.md"; then
...
echo "[9/62] Parent AGENTS.md §11.4 propagation anchor"
...
echo "[10/62] Parent CONSTITUTION.md §11.4 propagation anchor"
# Structural, per §11.4.201(7)(a) — see invariant 8's block comment for the
# measured declaration shapes and the §11.4.109 prefix-collision guard.
...
echo "[11/62] .claude/settings.json with PreToolUse guard hook"
...
echo "[12/62] docs/AGENT_GUARDRAILS.md SUBAGENT CONSTITUTIONAL PREAMBLE"
...
echo "[13/62] docs/AGENT_GUARDRAILS.md ORCHESTRATOR PRE-ACTION CHECKLIST"
if [[ -f "${GUARDRAILS_FILE}" ]] && \
...
echo "[14/62] constitution/scripts/hooks/guard-forbidden-commands.sh"
...
echo "[15/62] tests/hooks/test_guard_forbidden_commands.sh"
...
echo "[16/62] CM-MARKDOWN-EXPORT-SYNC: all-Markdown export freshness (§11.4.65)"
...
# THIS gate) while §11.4.18 itself was then unenforced (CM-SCRIPT-DOCS-SYNC —
# since implemented as invariant 61, BOB-242) — the author who
...
echo "[17/62] CM-WORKABLE-ITEMS-VALIDATE: workable-items validate (§11.4.93/§11.4.95)"
...
echo "[18/62] CM-WORKABLE-ITEMS-EXPORT-VALIDATE: workable-items-export.sh --check-only (§11.4.93/§11.4.65)"
...
echo "[19/62] CM-QA-DISCOVERY-LEDGER-FRESH: ledger fresh + counts aligned (§11.4.238)"
...
echo "[20/62] CM-QA-IS-THE-DISCOVERER: every out-of-band entry carries required fields (§11.4.238(C))"
if [[ ! -f "${LEDGER}" ]]; then
...
echo "[21/62] CM-COVENANT-114-238-PROPAGATION: §11.4.238 propagates (§11.4.227(B))"
PROP_FINDINGS=""
...
for mirror_pair in \
    ...
    if [[ ! -f "${path}" ]]; then
        ...
        # §11.4.227(B): a propagation gate counts BLOCK-STARTS, never bare
        # literals. `grep -qF '11.4.238'` was satisfied by ANY mention — a
        # citation, a cross-reference, a Status-summary line — so the gate
        # passed while the anchor BLOCK was entirely absent (measured
        # 2026-09-01: the project-root CLAUDE.md carries the literal once and
        # has 0 block-starts, exactly the shape the literal check cannot see).
        # The mirrors legitimately use a DIFFERENT heading shape than
        # Constitution.md's '### §11.4.238' — canonical-vs-mirror layer
        # variance is explicitly legitimate per §11.4.227(B) — so the pattern
        # is derived from the mirrors as they actually are (read 2026-09-01):
        # each carries exactly one '**§11.4.238 — …' bold block-start, and the
        # compact-list form '- §11.4.NNN — …' is the other legitimate mirror
        # block shape. '### ' is accepted too so a mirror that later adopts the
        # canonical heading is not a false null. The trailing ([^0-9.]|$) guard
        # stops a dotted sub-anchor or a longer number from prefix-matching.
        # The mid-line "Canonical authority: … §11.4.238." carrier is excluded
        # by the '^' anchor. Instrument proven seeing 2026-09-01: this pattern
        # returns 1 on each of the four mirrors, 1 for the control-needle
        # anchor 11.4.239, and 0 for the absent 11.4.999.
        MIRROR_BLOCKS=$(grep -cE '^(\*\*|### |- )§?11\.4\.238([^0-9.]|$)' "${path}" || true)
        if [[ "${MIRROR_BLOCKS}" -ne 1 ]]; then
            PROP_FINDINGS+="${label}: expected exactly 1 §11.4.238 block-start (^** / ^### / ^- + §11.4.238), got ${MIRROR_BLOCKS} (§11.4.227(B) block-integrity; a bare literal is a CARRIER, not a block)\n"
        fi
    fi
done
if [[ -n "${PROP_FINDINGS}" ]]; then
    ...
    pass "§11.4.238 propagates (Constitution block-start ×1, §11.4.157 mirror-set block-start ×1 each across 4 mirrors)"
fi
...
echo "[22/62] CM-WORKABLE-ITEMS-EXPORT-STEP1-REAL-INVOCATION: export step 1 really invoked (§11.4.238 RD2-41a)"
if [[ ! -f "${DOCS_CHAIN}" || ! -x "${DOCS_CHAIN}" ]]; then
...
#  53. CM-QBITTORRENT-WEBUI-CREDENTIALS: runs scripts/pre_build/check_cm_qbittorrent_webui_credentials.sh
#      — the qBittorrent WebUI must accept the documented admin/admin credentials AND that must be
#      REAL authentication, not a LocalHostAuth/subnet-whitelist bypass that accepts any password.
#      Retroactive catcher for the 2026-09-01 login outage (four compounding defects, zero gates).
#  54. CM-NO-TEST-TAG-DEBRIS: runs scripts/pre_build/check_cm_no_test_tag_debris.sh — the live
#      qBittorrent instance must carry no test-generated tags. Retroactive catcher for the
#      2026-09-01 audit that found 18 test-debris tags while every real torrent had tags=''.
#      SKIPs honestly when the stack is not running.
#  55. CM-NO-UNSCOPED-LIVE-DESTRUCTION: runs
#      scripts/pre_build/check_cm_no_unscoped_live_destruction.sh — no test or challenge may
#      delete from the live instance on the basis of an UNFILTERED full-instance listing
#      without diff-scoping against a baseline. Sibling of 54, one layer LEFT: 54 catches a
#      cleanup that removed too LITTLE, 55 a cleanup that removes too MUCH. Retroactive
#      catcher for tests/stress/test_search_stress.py::_purge_qbittorrent_torrents (a684b2f),
#      which listed every torrent and deleted the lot from an autouse fixture, wiping the
#      operator's library registration. Pure source check — needs no live stack.
#
...
echo "[23/62] CM-NO-PRODUCTION-MUTATION-RESIDUE: no mutation-marker residue in production sources (§11.4.84)"
...
echo "[24/62] CM-DOCS-CHAIN-ENGINE-VERIFY: docs_chain engine verify --all (§11.4.106)"
...
echo "[25/62] CM-RESOURCE-PRESSURE-SIGNATURE-CHECK: proactive host-pressure probe (§12.12, task #77/BOB-076)"
...
# itself (§11.4.89).
echo "[26/62] CM-BADGE-FRESHNESS-CHECK: README badges match live counts (§11.4.259, BOB-118)"
...
echo "[27/62] CM-KILLPG-PGID-GUARD: no unguarded process-group kill calls (§11.4.263, BOB-126)"
...
echo "[28/62] CM-TEST-MOCK-PID-EXPLICIT-INT: no unguarded subprocess-mock pid in tests (§11.4.263, BOB-126)"
...
# signal.
echo "[29/62] CM-TEST-MOCK-PID-PATCHED-WHEN-REAL-PID: no unpatched real-pid subprocess-mock in tests (§11.4.263, BOB-127)"
...
echo "[30/62] CM-BASH-UNIT-TESTS-EXECUTED: every tests/**/test_*.sh suite discovered from the tree actually runs (§11.4.226/§11.4.250)"
...
BASH_TEST_SELF_RECURSIVE=(
    ...
    "test_check_cm_zero_shortcomings_standing.sh"  # runs the full pre_build_verification.sh sweep (self-recursive)
...
run_const_gate() {
    ...
    # Extract the gate's VERDICT by STRUCTURE, never by substring
    # (§11.4.201(7)(a) — a token that MENTIONS a verdict is a CARRIER, not a
    # verdict). The prior pattern was '^(✅|❌|⏭)|(PASS|FAIL|SKIP)': in ERE the
    # leading '^' binds ONLY to the first alternation group, so the second group
    # was an UNANCHORED substring match, and the following `tail -n1` then took
    # the LAST such hit. A prose line the gate legitimately prints — e.g. a
    # mutation description "── M1: ... returns PASS immediately" or a note
    # "note: SKIP is not applicable here" — was therefore extracted AS the
    # verdict, discarding the real '✅ ... PASS' line above it and flipping the
    # exit-0 PASS/SKIP discrimination below. Three structural shapes are
    # accepted, each anchored at line start:
    #   1. an emoji-led verdict line ('✅ <GATE>: PASS — …'), the dominant shape;
    #   2. this gate's OWN name as the label ('<gname>: …') — the strongest
    #      anchor available here, since run_const_gate is handed the exact gate
    #      name and every wired gate's in-script GATE= equals it (verified
    #      2026-09-01 across all 9 wired gates). This keeps the propagation
    #      family's label-led summary line ('<GATE>: 7 PRESENT, 3
    #      POINTER-INHERITANCE-SKIP, 0 MISSING') extractable, which an
    #      emoji-only pattern would have dropped — a regression that would have
    #      turned that PASS into a spurious SKIP;
    #   3. a generic UPPERCASE label followed immediately by the verdict token
    #      ('<LABEL>: PASS|FAIL|SKIP'), covering gates that define no GATE= var.
    # Lowercase prose labels ('note: …') and non-alphabetic leads ('── M1: …')
    # match none of the three. gname is [A-Z0-9-]+ for every wired gate, so it
    # carries no ERE metacharacter; if a future gate name gains one, alternative
    # 2 simply stops matching and the honest fallback on the next line applies.
    verdict="$(grep -aE "^(✅|❌|⏭)|^${gname}:[[:space:]]|^[A-Z][A-Z0-9_.-]*:[[:space:]](PASS|FAIL|SKIP)([[:space:]]|$)" "${glog}" | tail -n1 || true)"
...
echo "[31/62] CM-COVENANT-PROPAGATION-BLOCK-INTEGRITY: 17 pack-unbound anchors (§11.4.227(B))"
...
echo "[32/62] CM-COVENANT-PROPAGATION-SUITE: 30 pack-driven anchors (§11.4.227(B), ADVISORY)"
...
echo "[39/62] CM-DANGEROUS-COMBINATION-FAIL-CLOSED: fail-open scan over first-party source (§11.4.252, ADVISORY)"
...
echo "[41/62] CM-OPENDESIGN-UI-SYSTEM: Angular frontend design-system audit (§11.4.162, ADVISORY)"
...
echo "[44/62] CM-HEALTHCHECK-COVERS-SERVED-PORTS: healthchecks probe every served port (§11.4.254, BOB-138)"
...
# --- Invariant 45: CM-OWNERSHIP-INVARIANTS (§11.4.201, FR-011 of feature 002) ---
...
# BLOCKING.
echo "[45/62] CM-OWNERSHIP-INVARIANTS: operator-owned writes not reverted (§11.4.201, FR-011)"
...
# BLOCKING.
echo "[46/62] CM-PLUGIN-COUNT: documented plugin counts match their derivation (BOB-149)"
...
# BLOCKING.
echo "[47/62] CM-GO-TOOLCHAIN-MATCHES-BUILDER: Dockerfile builder satisfies go.mod (BOB-153)"
...
echo "[48/62] CM-RUNTIME-DEPS-PARITY: test stack and production run the same deps (BOB-154)"
...
# ---------------------------------------------------------------------------
echo "[49/62] CM-CLOSURE-SEAM-BINDS: done-but-open rows found mechanically (§11.4.226)"
...
# --- Invariant 50: CM-EXPORT-CHARSET-VALID (BOB-169 acceptance (d), §11.4.238) ---
#
# The export regime asserted PRESENCE and MTIME, never VALIDITY. Measured
# 2026-08-23: 301 of 334 generated exports were charset-less fragments and 281
# PDFs carried 12,629 corrupted lines, while every gate stayed green. The defect
...
# author's own set).
echo "[50/62] CM-EXPORT-CHARSET-VALID: exports declare a charset AND the ratchet is current (§11.4.238)"
...
echo "[51/62] CM-SERVED-BUNDLE-FRESH: the served dashboard bundle matches its sources (BOB-183)"
...
# --if-present encodes exactly that, and relaxes NOTHING else: a bundle that IS
# present and stale still FAILS here, and a BLIND input enumeration still FAILS.
# The POST-build seam (scripts/install.sh stage 3) runs the same gate WITHOUT the
# flag and refuses on absence, which is where absence genuinely means "nothing to
# serve" (§11.4.135 seam-dependent PENDING semantics).
if [[ -x "${PROJECT_ROOT}/scripts/pre_build/check_cm_served_bundle_fresh.sh" ]]; then
    if bash "${PROJECT_ROOT}/scripts/pre_build/check_cm_served_bundle_fresh.sh" --if-present >/dev/null 2>&1; then
        pass "CM-SERVED-BUNDLE-FRESH: served bundle corresponds to frontend sources (or is not yet built)"
    else
        fail "CM-SERVED-BUNDLE-FRESH: the served bundle diverges from frontend/src, or the input scan went blind"
        bash "${PROJECT_ROOT}/scripts/pre_build/check_cm_served_bundle_fresh.sh" --if-present 2>&1 | sed 's/^/        /' >&2 || true
    fi
else
    # §11.4.3 honest SKIP — never a silent pass.
...
echo "[52/62] CM-WORKABLE-ITEMS-BINARY-FRESH: the shipped workable-items binary matches its sources (BOB-188)"
...
# every consumer, and a shipped-but-stale artifact must block here. The gate's own
# absence branch stays honest: no binary at all is a §11.4.69 artifact_not_yet_built
# SKIP inside the gate, so a checkout that genuinely ships nothing is not refused.
if [[ -x "${PROJECT_ROOT}/scripts/pre_build/check_cm_workable_items_binary_fresh.sh" ]]; then
    if bash "${PROJECT_ROOT}/scripts/pre_build/check_cm_workable_items_binary_fresh.sh" >/dev/null 2>&1; then
        pass "CM-WORKABLE-ITEMS-BINARY-FRESH: shipped binary corresponds to its Go sources"
    else
        fail "CM-WORKABLE-ITEMS-BINARY-FRESH: the shipped binary is stale, unfingerprinted, or the input scan went blind"
        bash "${PROJECT_ROOT}/scripts/pre_build/check_cm_workable_items_binary_fresh.sh" 2>&1 | sed 's/^/        /' >&2 || true
    fi
else
    # §11.4.3 honest SKIP — never a silent pass.
    echo "  SKIP: scripts/pre_build/check_cm_workable_items_binary_fresh.sh absent or not executable"
fi

# --- Invariant 53: CM-QBITTORRENT-WEBUI-CREDENTIALS (§11.4.239, login repair 2026-09-01) ---
# The qBittorrent WebUI must be reachable with the documented admin/admin
# credentials, and that must be REAL authentication rather than a bypass.
#
# Retroactive catcher for the 2026-09-01 outage in which FOUR defects lined up
# and no gate could see any of them: credentials authored into a config path
# the image never reads; WEBUI_USERNAME/WEBUI_PASSWORD set in compose although
# lscr.io/linuxserver/qbittorrent implements neither; start.sh's credential
# writer being replace-only so it wrote nothing into a fresh config; and
# qBittorrent 5.x consequently minting a random temporary password every boot.
#
# It ALSO pins the security half: the first repair attempt enforced
# WebUI\LocalHostAuth=false + AuthSubnetWhitelistEnabled=true, which does not
# relax the brute-force ban but DISABLES authentication for loopback and all
# RFC1918 — measured: a wrong password then returned HTTP 204 (accepted).
#
# BLOCKING: availability + auth integrity of the primary user-facing
# capability (§11.4.239 critical-invariant work class).
echo "[53/62] CM-QBITTORRENT-WEBUI-CREDENTIALS: WebUI credentials enforced and auth not bypassed (§11.4.239)"
...
if [[ ! -f "${QBT_CREDS_GATE}" ]]; then
    ...
    QBT_CREDS_LOG="$(mktemp)"; QBT_CREDS_RC=0
    timeout "${CONST_GATE_TIMEOUT}" bash "${QBT_CREDS_GATE}" >"${QBT_CREDS_LOG}" 2>&1 || QBT_CREDS_RC=$?
    if [[ "${QBT_CREDS_RC}" -eq 0 ]]; then
        pass "CM-QBITTORRENT-WEBUI-CREDENTIALS: $(tail -n1 "${QBT_CREDS_LOG}" || true)"
    elif [[ "${QBT_CREDS_RC}" -eq 124 ]]; then
        fail "CM-QBITTORRENT-WEBUI-CREDENTIALS: exceeded the ${CONST_GATE_TIMEOUT}s budget (a timeout is not a pass)"
    else
        fail "CM-QBITTORRENT-WEBUI-CREDENTIALS: exit ${QBT_CREDS_RC} — WebUI credentials are not enforced, or authentication is bypassed"
        echo "        --- gate output ---"
        sed 's/^/        /' "${QBT_CREDS_LOG}"
        echo "        --- end ---"
    fi
...
# --- Invariant 54: CM-NO-TEST-TAG-DEBRIS (§11.4.14, tagging audit 2026-09-01) ---
# The live qBittorrent instance must carry no test-generated tags.
#
# Retroactive catcher: an operator audit found EIGHTEEN tags in the WebUI, all
# of them debris (`boba-bridge-{go,red,neg}-<uuid>` from an integration test
# that never cleaned up, plus two probe tags), while every REAL torrent carried
# tags=''. The only tags the operator could see were the test suite's litter.
#
# A cleanup fixture was added to that suite, but a teardown FAILURE only prints
# to stderr and the run still reports green — so a cleanup that silently broke
# would re-accumulate debris with nothing watching. This gate is the
# independent observer: it reads the LIVE instance rather than trusting the
# cleanup code that was supposed to run (review finding M-e).
#
# BLOCKING, deliberately. A WARN would restore the exact invisibility this
...
echo "[54/62] CM-NO-TEST-TAG-DEBRIS: no test-generated tags in the live qBittorrent (§11.4.14)"
...
if [[ ! -f "${TAG_DEBRIS_GATE}" ]]; then
    ...
    TAG_DEBRIS_LOG="$(mktemp)"; TAG_DEBRIS_RC=0
    timeout "${CONST_GATE_TIMEOUT}" bash "${TAG_DEBRIS_GATE}" >"${TAG_DEBRIS_LOG}" 2>&1 || TAG_DEBRIS_RC=$?
    if [[ "${TAG_DEBRIS_RC}" -eq 0 ]]; then
        pass "CM-NO-TEST-TAG-DEBRIS: $(tail -n1 "${TAG_DEBRIS_LOG}" || true)"
    elif [[ "${TAG_DEBRIS_RC}" -eq 124 ]]; then
        fail "CM-NO-TEST-TAG-DEBRIS: exceeded the ${CONST_GATE_TIMEOUT}s budget (a timeout is not a pass)"
    else
        fail "CM-NO-TEST-TAG-DEBRIS: exit ${TAG_DEBRIS_RC} — test debris is present in the live instance"
        echo "        --- gate output ---"
        sed 's/^/        /' "${TAG_DEBRIS_LOG}"
        echo "        --- end ---"
    fi
    rm -f "${TAG_DEBRIS_LOG}"
fi
...
# ---------------------------------------------------------------------------
# 55. CM-NO-UNSCOPED-LIVE-DESTRUCTION
#
# SIBLING of gate 54, one layer LEFT. Gate 54 observes the live instance for
# debris a cleanup FAILED to remove; gate 55 reads the SOURCE for cleanups
# that remove TOO MUCH.
#
# The escape it closes (§11.4.238 — an agent found it, no gate did):
# tests/stress/test_search_stress.py::_purge_qbittorrent_torrents (a684b2f)
# listed EVERY torrent in the instance and deleted the lot, from an autouse
# fixture running before every stress test. On the operator's live instance
# that de-registered their whole library — files survived (deleteFiles=false),
# session/categories/tags/ratio/seeding state did not.
#
# BLOCKING, deliberately: this is a data-safety class, and a WARN on a rule
...
echo "[55/62] CM-NO-UNSCOPED-LIVE-DESTRUCTION: test-tree deletes are diff-scoped (§9 / §11.4.14)"
...
if [[ ! -f "${UNSCOPED_GATE}" ]]; then
    ...
    UNSCOPED_LOG="$(mktemp)"; UNSCOPED_RC=0
    timeout "${CONST_GATE_TIMEOUT}" bash "${UNSCOPED_GATE}" >"${UNSCOPED_LOG}" 2>&1 || UNSCOPED_RC=$?
    if [[ "${UNSCOPED_RC}" -eq 0 ]]; then
        pass "CM-NO-UNSCOPED-LIVE-DESTRUCTION: $(tail -n1 "${UNSCOPED_LOG}" || true)"
    elif [[ "${UNSCOPED_RC}" -eq 124 ]]; then
        fail "CM-NO-UNSCOPED-LIVE-DESTRUCTION: exceeded the ${CONST_GATE_TIMEOUT}s budget (a timeout is not a pass)"
    elif [[ "${UNSCOPED_RC}" -eq 2 ]]; then
        fail "CM-NO-UNSCOPED-LIVE-DESTRUCTION: harness error — the scan could not run, so the tree is UNVERIFIED (not clean)"
        echo "        --- gate output ---"
        sed 's/^/        /' "${UNSCOPED_LOG}"
        echo "        --- end ---"
    else
        fail "CM-NO-UNSCOPED-LIVE-DESTRUCTION: exit ${UNSCOPED_RC} — a test deletes live state without diff-scoping"
        echo "        --- gate output ---"
        sed 's/^/        /' "${UNSCOPED_LOG}"
        echo "        --- end ---"
    fi
...
# ---------------------------------------------------------------------------
# 56. CM-GITIGNORE-SWALLOW-GUARD
#
# BOB-212: a NEW first-party SOURCE file whose name matches a .gitignore
# deny-all glob (*credentials*, *creds*, *secrets*, *_password*, *_user*,
# ...) is silently swallowed — `git add` refuses it and `git status` shows
# NOTHING, so the author gets no signal it did not land. Measured TWICE this
# project's own history: a RED test file and a legitimate feature test both
# vanished this way, caught only because the authoring agent went looking
# for the file it had just written. Narrowing the globs was measured and
# DISPROVEN (it leaks a real credential file already in this tree,
# download-proxy/qbittorrent_creds.json) — the globs stay exactly as they
# are; this gate adds the LOUD refusal on top, scoped to first-party
...
echo "[56/62] CM-GITIGNORE-SWALLOW-GUARD: no first-party source file is silently swallowed by .gitignore (§11.4.201(6), BOB-212)"
...
if [[ ! -f "${SWALLOW_GATE}" ]]; then
    ...
    SWALLOW_LOG="$(mktemp)"; SWALLOW_RC=0
    timeout "${CONST_GATE_TIMEOUT}" bash "${SWALLOW_GATE}" "${PROJECT_ROOT}" >"${SWALLOW_LOG}" 2>&1 || SWALLOW_RC=$?
    if [[ "${SWALLOW_RC}" -eq 0 ]]; then
        pass "CM-GITIGNORE-SWALLOW-GUARD: $(tail -n1 "${SWALLOW_LOG}" || true)"
    elif [[ "${SWALLOW_RC}" -eq 124 ]]; then
        fail "CM-GITIGNORE-SWALLOW-GUARD: exceeded the ${CONST_GATE_TIMEOUT}s budget (a timeout is not a pass)"
    elif [[ "${SWALLOW_RC}" -eq 2 ]]; then
        fail "CM-GITIGNORE-SWALLOW-GUARD: harness error — the scan could not run, so the tree is UNVERIFIED (not clean)"
        echo "        --- gate output ---"
        sed 's/^/        /' "${SWALLOW_LOG}"
        echo "        --- end ---"
    else
        fail "CM-GITIGNORE-SWALLOW-GUARD: exit ${SWALLOW_RC} — a first-party source file is silently swallowed by .gitignore"
        echo "        --- gate output ---"
        sed 's/^/        /' "${SWALLOW_LOG}"
        echo "        --- end ---"
    fi
...
# ---------------------------------------------------------------------------
# 57. CM-NO-FAIL-OPEN-SKIP
#
# BOB-161 / §11.4.69: the constitution names this gate as mandatory and the
# project did not have it. BOB-092 removed two fail-open skips from
# tests/e2e/test_live_stack_evidence.py one at a time — each turned evidence
# the service had ANSWERED into a PASS-counting SKIP — and both were found by
# an agent reading code, not by the regime (§11.4.238). The detector is an
# AST engine with an in-path control needle; existing findings are ratcheted
# as a finding SET (remediation: BOB-192). Self-test with golden-TRUE,
# golden-FALSE-with-carrier fixtures and paired §1.1 mutations:
# tests/pre_build/test_check_cm_no_fail_open_skip.sh.
#
# BLOCKING: a pure source check needing no live stack, so it cannot be
# quietly skipped by running with the containers down.
echo "[57/62] CM-NO-FAIL-OPEN-SKIP: no test turns an answered response into a SKIP (§11.4.69, BOB-161)"
...
if [[ ! -f "${FAILOPEN_GATE}" ]]; then
    ...
    FAILOPEN_LOG="$(mktemp)"; FAILOPEN_RC=0
    timeout "${CONST_GATE_TIMEOUT}" bash "${FAILOPEN_GATE}" >"${FAILOPEN_LOG}" 2>&1 || FAILOPEN_RC=$?
    if [[ "${FAILOPEN_RC}" -eq 0 ]]; then
        pass "CM-NO-FAIL-OPEN-SKIP: $(tail -n1 "${FAILOPEN_LOG}" || true)"
    elif [[ "${FAILOPEN_RC}" -eq 124 ]]; then
        fail "CM-NO-FAIL-OPEN-SKIP: exceeded the ${CONST_GATE_TIMEOUT}s budget (a timeout is not a pass)"
    elif [[ "${FAILOPEN_RC}" -eq 2 ]]; then
        fail "CM-NO-FAIL-OPEN-SKIP: harness error — the scan could not run or its control needle was not seen, so the tree is UNVERIFIED (not clean)"
        echo "        --- gate output ---"
        sed 's/^/        /' "${FAILOPEN_LOG}"
        echo "        --- end ---"
    else
        fail "CM-NO-FAIL-OPEN-SKIP: exit ${FAILOPEN_RC} — a NEW fail-open skip, or a STALE baseline row to remove"
        echo "        --- gate output ---"
        grep -E '^(NEW|STALE|FAIL)' "${FAILOPEN_LOG}" | sed 's/^/        /' || true
        echo "        --- end ---"
    fi
    rm -f "${FAILOPEN_LOG}"
fi
...
echo "[58/62] CM-MD-EXPORT-TWINS-COMMITTABLE: tracked .md export twins are never gitignore-swallowed (§11.4.65, BOB-219)"
...
# ---------------------------------------------------------------------------
# 59. CM-ZERO-SHORTCOMINGS-STANDING (ADVISORY, spec 003 US3 / FR-009)
#
# Runs the zero-shortcomings audit's standing-check and prints its last line.
...
echo "[59/62] CM-ZERO-SHORTCOMINGS-STANDING: three-surface audit standing-check (ADVISORY)"
zsc_standing_stage() {
...
# ---------------------------------------------------------------------------
# 60. CM-PREBUILD-INVOKED-EXECUTABLES-TRACKED (BOB-231, §11.4.30/§11.4.215/§11.4.227)
#
# Every executable this sweep invokes (plus the helpers those scripts call one
# level down) must be git-tracked; otherwise a fresh clone runs a different —
# or no — gate. BOB-227 measured 3 of 4 files of one gate untracked. BLOCKING
# for invoked gate files; untracked tests/**/test_*.sh suites (which invariant
# 30 discovers) are reported as ADVISORY WARNs only, because a work-in-progress
# test on a developer machine is normal. Self-tested by
# tests/pre_build/test_check_prebuild_invoked_executables_tracked.sh.
echo "[60/62] CM-PREBUILD-INVOKED-EXECUTABLES-TRACKED: every executable this sweep invokes is git-tracked (BOB-231)"
INVTRK_GATE="${PROJECT_ROOT}/scripts/pre_build/check_prebuild_invoked_executables_tracked.sh"
if [[ ! -f "${INVTRK_GATE}" ]]; then
    fail "CM-PREBUILD-INVOKED-EXECUTABLES-TRACKED: gate script missing at scripts/pre_build/check_prebuild_invoked_executables_tracked.sh"
else
    INVTRK_LOG="$(mktemp)"; INVTRK_RC=0
    timeout "${CONST_GATE_TIMEOUT}" bash "${INVTRK_GATE}" "${PROJECT_ROOT}" >"${INVTRK_LOG}" 2>&1 || INVTRK_RC=$?
    if [[ "${INVTRK_RC}" -eq 0 ]]; then
        pass "CM-PREBUILD-INVOKED-EXECUTABLES-TRACKED: $(tail -n1 "${INVTRK_LOG}" || true)"
        grep -F 'WARN (advisory)' "${INVTRK_LOG}" | sed 's/^/    /' || true
    elif [[ "${INVTRK_RC}" -eq 124 ]]; then
        fail "CM-PREBUILD-INVOKED-EXECUTABLES-TRACKED: exceeded the ${CONST_GATE_TIMEOUT}s budget (a timeout is not a pass)"
    elif [[ "${INVTRK_RC}" -eq 2 ]]; then
        fail "CM-PREBUILD-INVOKED-EXECUTABLES-TRACKED: harness error — the enumeration could not run or saw nothing, so the sweep is UNVERIFIED (not clean)"
        sed 's/^/        /' "${INVTRK_LOG}"
    else
        fail "CM-PREBUILD-INVOKED-EXECUTABLES-TRACKED: exit ${INVTRK_RC} — the sweep invokes executable(s) that are not git-tracked"
        echo "        --- gate output ---"
        sed 's/^/        /' "${INVTRK_LOG}"
        echo "        --- end ---"
    fi
...
# ---------------------------------------------------------------------------
# 61. CM-SCRIPT-DOCS-SYNC (§11.4.18, BOB-242) — was a registered deferral (BOB-223)
#
# Every *.sh/*.bash under scripts/ needs docs/scripts/<name>.md, and the guide
# must not lag the script (last commit touching each; an uncommitted edit counts
# as "now"). Brownfield adoption is a MONOTONE-DECREASING RATCHET against
# scripts/pre_build/cm_script_docs_sync.baseline (seeded 2026-09-26: 63 rows =
# 52 missing guides + 11 guides behind their script): a NEW finding or a
# RETIRED baseline row FAILs; baselined rows are reported. BLOCKING. The ratchet
# is the repository's established default for set-based gates and is open to an
# operator decision (§11.4.66). Self-tested by
# tests/pre_build/test_check_cm_script_docs_sync.sh.
echo "[61/62] CM-SCRIPT-DOCS-SYNC: every script has an in-sync docs/scripts guide, ratcheted (§11.4.18, BOB-242)"
SDS_GATE="${PROJECT_ROOT}/scripts/pre_build/check_cm_script_docs_sync.sh"
if [[ ! -f "${SDS_GATE}" ]]; then
    fail "CM-SCRIPT-DOCS-SYNC: gate script missing at scripts/pre_build/check_cm_script_docs_sync.sh"
else
    SDS_LOG="$(mktemp)"; SDS_RC=0
    timeout "${CONST_GATE_TIMEOUT}" bash "${SDS_GATE}" "${PROJECT_ROOT}" >"${SDS_LOG}" 2>&1 || SDS_RC=$?
    if [[ "${SDS_RC}" -eq 0 ]]; then
        pass "CM-SCRIPT-DOCS-SYNC: $(tail -n1 "${SDS_LOG}" || true)"
    elif [[ "${SDS_RC}" -eq 124 ]]; then
        fail "CM-SCRIPT-DOCS-SYNC: exceeded the ${CONST_GATE_TIMEOUT}s budget (a timeout is not a pass)"
    elif [[ "${SDS_RC}" -eq 2 ]]; then
        fail "CM-SCRIPT-DOCS-SYNC: harness error — the walk could not run or saw nothing, so the tree is UNVERIFIED (not clean)"
        sed 's/^/        /' "${SDS_LOG}"
    else
        fail "CM-SCRIPT-DOCS-SYNC: exit ${SDS_RC} — new script/guide drift or a retired baseline row"
        echo "        --- gate output ---"
        grep -v '^BASELINED ' "${SDS_LOG}" | sed 's/^/        /'
        echo "        --- end ---"
    fi
    rm -f "${SDS_LOG}"
fi
...
# ---------------------------------------------------------------------------
# 62. CM-DOC-REVISION-HEADER-PRESENT (§11.4.44, BOB-242) — was a registered deferral (BOB-223)
#
# Every docs/scripts/*.md carries **Revision:** <positive int> and
# **Last modified:** <ISO 8601 UTC> in its first 15 lines. Measured 2026-09-26:
# 1 of 45 guides failed (an offset timestamp), corrected in the same change, so
# this is a hard floor with no baseline. BLOCKING. Self-tested by
# tests/pre_build/test_check_cm_doc_revision_header_present.sh.
echo "[62/62] CM-DOC-REVISION-HEADER-PRESENT: every docs/scripts guide carries a revision header (§11.4.44, BOB-242)"
DRH_GATE="${PROJECT_ROOT}/scripts/pre_build/check_cm_doc_revision_header_present.sh"
if [[ ! -f "${DRH_GATE}" ]]; then
    fail "CM-DOC-REVISION-HEADER-PRESENT: gate script missing at scripts/pre_build/check_cm_doc_revision_header_present.sh"
else
    DRH_LOG="$(mktemp)"; DRH_RC=0
    timeout "${CONST_GATE_TIMEOUT}" bash "${DRH_GATE}" "${PROJECT_ROOT}" >"${DRH_LOG}" 2>&1 || DRH_RC=$?
    if [[ "${DRH_RC}" -eq 0 ]]; then
        pass "CM-DOC-REVISION-HEADER-PRESENT: $(tail -n1 "${DRH_LOG}" || true)"
    elif [[ "${DRH_RC}" -eq 124 ]]; then
        fail "CM-DOC-REVISION-HEADER-PRESENT: exceeded the ${CONST_GATE_TIMEOUT}s budget (a timeout is not a pass)"
    elif [[ "${DRH_RC}" -eq 2 ]]; then
        fail "CM-DOC-REVISION-HEADER-PRESENT: harness error — the walk could not run or saw nothing, so the tree is UNVERIFIED (not clean)"
        sed 's/^/        /' "${DRH_LOG}"
    else
        fail "CM-DOC-REVISION-HEADER-PRESENT: exit ${DRH_RC} — guide(s) without a valid §11.4.44 header"
        echo "        --- gate output ---"
        sed 's/^/        /' "${DRH_LOG}"
        echo "        --- end ---"
    fi
...
#
# Implementation for BOTH was tracked as BOB-242 and LANDED 2026-09-26 as
# invariants 61 (CM-SCRIPT-DOCS-SYNC) and 62 (CM-DOC-REVISION-HEADER-PRESENT)
# above, so both DEFERRED lines were removed. The register itself stays: a
# future named-but-unimplemented gate is registered here, and
# tests/pre_build/test_gate_debt_register_markers.sh asserts each of these two
# names is EITHER implemented (label + gate script invoked + file present) OR
# deferred — never silently absent.
echo
echo "[GATE-DEBT REGISTER] §11.4.227(A) registered deferrals (informational, non-blocking, exit-0-contributing):"
echo "  (none) — CM-SCRIPT-DOCS-SYNC and CM-DOC-REVISION-HEADER-PRESENT are IMPLEMENTED as invariants 61/62 (BOB-242)"
